...
**W1-24 local implementation (2026-07-14):** TikTok/Shopify publish and distribution status now reuse strict credential validators, contain no runtime mock fallback, require exact `simulated` truth, and separate deterministic rejection from ambiguous external outcome. `PublishAttemptService` preserves W1-23 single-use/no-retry authority; status returns only stable 503/502 or trusted real 200. Read `docs/superpowers/specs/2026-07-14-publish-connector-truth-design.md`, `docs/superpowers/plans/2026-07-14-publish-connector-truth.md`, and `docs/runbooks/publish-connector-truth.md` before extending this behavior. The W1-24 checkpoint is `implementation_complete_local / independent_review_pending` with `independent_review=false`; its W1-25 receipt/env gap is closed by the next local checkpoint, while W1-26 live publish remains separate. Production is unchanged and no credentialed real publish succeeded. Two early fixture-only RED network escapes led to the permanent test client-construction guard.
...
**Provider-off release-candidate closure (2026-07-20):** W1-31 billing reconciliation is retired and every execute entrypoint now fails before provider client construction. The release path requires a clean, exact `origin/main` source, three SHA-tagged reviewed images, dynamic public-table backup/restore with Alembic parity, explicit migration, production PostgreSQL fail-closed readiness, and a versioned release. Shared nginx and `portal_auth` remain online; deploy only backs up/replaces the reviewed AI Video location snippet, runs `nginx -t`, reloads, and restores/reloads on rollback. Current evidence is local/disposable only; actual GitHub artifacts, restricted remote dry-run, deploy, and L3 acceptance remain pending.
