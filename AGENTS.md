...
**W5-04 first governed live window and H3 local repair (2026-07-24):** Production SHA `0bb5022e2f03c212f1681789428e0e1c42b13255` passed exact identity, credential-presence, funded-account, contract, and same-key preflight for one Fast submit capped at one DeepSeek Flash job, one Seedance 2 video job, `$3.15`, and retry 0. The durable marker consumed authority and exactly one POST returned HTTP 500 `submission_initialization_failed`; no poll or resend ran. Durable readback is `fast_initialization_failed`, provider-cost account absent, attempts 0, no artifact, and publish/delivery false; provider-off restore passed. Root cause was the production window omitting mandatory server-owned `PROVIDER_JOB_BUDGET_USD` before account initialization. H3 now freezes one exact `O_EXCL`/`0600` plan snapshot, derives the canonical cap and SHA from those bytes, copies the same snapshot, and revalidates installed plan digest/budget before marker creation. Independent review accepted one Medium plan-swap TOCTOU, the main thread fixed it with a deterministic swap regression, and the same reviewer returned `PASS / APPROVE`, `accepted_actionable_findings=0`. Final local evidence is affected `472 passed, 1 deselected`, full backend `4487 passed, 9 skipped, 24 deselected`, source Pyright `0 errors`, and Ruff/ratchet/Bash/diff/secret gates green. The H3 repair is not committed or deployed; production remains provider-off at the prior SHA. Any corrected live attempt requires separate exact release promotion and a wholly fresh L4 packet—never reuse the consumed activation, binding, marker, or key.
