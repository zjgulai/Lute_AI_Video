...
1. **业务用户**（部门用户、外部租户）：通过前端 SettingsPanel 输入 API Key → 调创作 API（`/scenario/*`、`/fast/generate`、`/portfolio/*`）。Key 跟着每个请求走 `X-API-Key` header，**无状态**，无浏览器 session 概念。`ai_video_demo_2026` 属于本地/测试打包 key，不作为生产公共只读 key。
...
| 状态 | 无状态 | 有状态 |
| 测试模式 | 显式 `TEST_BUNDLE_KEY`，生产需 `ALLOW_TEST_BUNDLE_KEY=1` | 无 demo |
...
- 校验顺序：
  ...
  2. 优先查 `api_keys` 表 sha256(key) 是否存在 → 通过则返回 `AuthContext(tenant_id, permissions, key_type)`
  3. 显式 `TEST_BUNDLE_KEY` 可作为本地/测试打包 key；生产环境必须同时设置 `ALLOW_TEST_BUNDLE_KEY=1`
  4. 旧 `API_KEY` fallback 只默认允许开发环境；生产环境未显式放开时返回 401
  5. 认证上下文写入 contextvars，状态读写接口按 `tenant_id` 拒绝跨租户访问
