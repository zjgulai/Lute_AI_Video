...
| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | local_no_provider_passed | 已完成本地/fixture readiness；生产只读与真实事件未执行 | `109 passed` 覆盖 metrics router/repository/poller、webhook manager、portfolio hook 与 analytics agent；证据等级仅为 `L2-fixture-or-dry-run`；TikTok/Shopify metrics fetcher stub、webhook.site、生产 PG/Alembic/scheduler、生产 `/metrics/*` readback 与前端真数据仍未验证 |
...
agent 相关目标集通过，结果 `109 passed`。但生产是否被调度、Alembic 的 `video_metrics` 表
是否真的 `alembic upgrade head` 过、PG 与 SQLite 双路是否在生产生效仍未做生产只读验证。
`MetricsPoller` 的 TikTok/Shopify fetcher 仍是 stub，前端 `PerformanceDashboard` 显示真数据
未验证。
