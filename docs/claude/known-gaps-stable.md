...
| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | platform_metrics_pull_blocked_by_connector_mapping_and_no_active_post | 本地/fixture readiness、生产 authenticated dashboard readback、`/dashboard` 前端 read-only、`/metrics/pull` fail-closed 生产同步均已完成；`TODO-P2-1L1/L2/L3` 已完成本地 no-provider 的 platform fetcher / poller ingestion / webhook receiver contract；`TODO-P2-1L4` 已完成生产 read-only regression；`TODO-P2-1L5R` 已完成 1 次 authorized live webhook-only synthetic `audit.completed` receiver readback；`TODO-P2-1L6A` 已完成本地 connector contract / error classification / dry-run readiness 修复，但 TikTok/Shopify 真实 metrics API mapping 仍未实现，生产 active post 候选仍为 `0`；真实 metrics pull、startup scheduler 与平台 metrics 数据流仍未执行 | `TODO-P2-1L1/L2` 本地目标测试 `6 passed`；`TODO-P2-1L3` webhook contract `18 passed`；metrics / webhook / analytics no-provider 回归集 `118 passed`；`TODO-P2-1L4` 生产 GET `/api/health`、`/api/dashboard/overview?days=7`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent` 均为 `200`；`TODO-P2-1L5R` receiver readback 新增 `1` 次 POST；`TODO-P2-1L6A` 本地目标测试 `19 passed`，生产只读 `/api/health=200`、`metrics_pull_enabled=false`、`active_post_count=0`；不得外推为真实 TikTok/Shopify metrics pull、生产 scheduler、publish、delivery acceptance 或 approved brand token write |
