...
2026-06-18 P2 active-post source contract：`TODO-P2-1L6E/L6F/L6G` 已继续收口到 no-pull readiness。`L6E` 已把 API mapping 相关已合并文件同步到生产并验证 `METRICS_PULL_ENABLED=false`、`dry_run_due_posts()` 可执行且不调用 platform fetcher；`L6F` 生产只读探查显示 `video_metrics.total=0`、`publish_logs.total=0`，状态为 `blocked_by_no_active_post_source`。本轮 `L6G` 只做本地合同修复：`VideoMetricsRepository.get_active_post_source_summary()` 明确 `video_metrics` 是当前唯一 live pull active source，`publish_logs` 只作为只读 source evidence 且因缺少 `video_id/scenario/published_at` 不自动成为 pull candidate；manual allowlist 只过滤已存在 `video_metrics.post_id`，manual seed 被标记为需要另行 DB write 授权。`MetricsPoller.dry_run_due_posts()` 现在返回 `source_contract`，无 source 时输出 `blocked_by_no_active_post_source`，只有 `publish_logs` 候选时输出 `blocked_by_publish_logs_not_active_source`；source inspection 只声明 `active_source_available`，`live_pull_ready` 仍 fail-closed，需 due-post、allowlist 和显式授权分别通过。目标验证：`ruff` 目标文件通过，`DATABASE_URL= HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 .venv/bin/pytest tests/test_metrics_poller.py tests/test_metrics_dashboard.py -q` 结果 `31 passed`。证据等级为 `L2-local-no-provider-contract`；仍未生产部署、未同步本轮 L6G 代码到生产、未调用 `/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics API、未 webhook 外发、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。
...
| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | platform_metrics_pull_blocked_by_no_active_post_source_and_shopify_post_dimension | 本地/fixture readiness、生产 authenticated dashboard readback、`/dashboard` 前端 read-only、`/metrics/pull` fail-closed 生产同步均已完成；`TODO-P2-1L1/L2/L3` 已完成本地 no-provider 的 platform fetcher / poller ingestion / webhook receiver contract；`TODO-P2-1L4` 已完成生产 read-only regression；`TODO-P2-1L5R` 已完成 1 次 authorized live webhook-only synthetic `audit.completed` receiver readback；`TODO-P2-1L6A` 已完成本地 connector contract / error classification / dry-run readiness 修复；`TODO-P2-1L6C` 已实现 TikTok / Shopify 真实 API mapping code path 并通过 mocked HTTP/no-provider 合同测试；`TODO-P2-1L6G` 已明确 active source 合同，但生产 `video_metrics` 与 `publish_logs` 候选均为 `0`，ShopifyQL post-level 过滤维度未确认；真实 metrics pull、startup scheduler 与平台 metrics 数据流仍未执行 | `TODO-P2-1L1/L2` 本地目标测试 `6 passed`；`TODO-P2-1L3` webhook contract `18 passed`；metrics / webhook / analytics no-provider 回归集 `118 passed`；`TODO-P2-1L4` 生产 GET `/api/health`、`/api/dashboard/overview?days=7`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent` 均为 `200`；`TODO-P2-1L5R` receiver readback 新增 `1` 次 POST；`TODO-P2-1L6A` 本地目标测试 `19 passed`；`TODO-P2-1L6C` 目标测试 `29 passed`；`TODO-P2-1L6G` 目标测试 `31 passed`；不得外推为真实 TikTok/Shopify metrics pull、生产 scheduler、publish、delivery acceptance 或 approved brand token write |
