...
2026-06-17 P2 metrics pull fail-closed 本地 guard：`TODO-P2-1J` 已在本地分支 `codex/metrics-pull-fail-closed-guard` 增加 `METRICS_PULL_ENABLED` 显式开关，使 `POST /metrics/pull` 默认返回 `403 Metrics pull is disabled`，避免真实 platform fetcher / scheduler 尚未闭环时被任意生产 key 触发；只有显式启用后才会调用 `MetricsPoller.pull_all()`。新增测试覆盖默认禁用不调用 poller、显式启用才调用 poller。验证：`ruff check src/config.py src/routers/metrics.py tests/test_metrics_dashboard.py tests/test_metrics_poller.py` 通过；`DATABASE_URL= .venv/bin/pytest tests/test_metrics_dashboard.py tests/test_metrics_poller.py -q` 结果 `8 passed`；P2-1 原 no-provider 目标集复跑 `DATABASE_URL= .venv/bin/pytest tests/test_metrics_dashboard.py tests/test_video_metrics_integration.py tests/test_metrics_repository.py tests/test_metrics_poller.py tests/test_webhook_manager.py tests/test_portfolio_mechanism.py tests/test_agents.py -q` 结果 `112 passed`。证据等级仍为 `L2-fixture-or-dry-run`；本轮未推送、未创建 PR、未同步生产、未重启服务、未调用 `/metrics/pull`、未调用真实 TikTok/Shopify platform metrics、未配置 webhook 接收端、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。
...
| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | local_pull_fail_closed_guard_implemented | 本地/fixture readiness 已通过；API key `expires_at` 与 dashboard contract 修复均已同步生产并通过 authenticated GET / browser readback；`/dashboard` 自然访问入口已合并并完成 frontend-only 生产同步；本地已为 `/metrics/pull` 增加 `METRICS_PULL_ENABLED` fail-closed guard；真实 metrics 事件链仍未执行 | `112 passed` 覆盖 metrics router/repository/poller、webhook manager、portfolio hook、analytics agent 与 `/metrics/pull` 默认禁用/显式启用合同；生产 `/api/health` 显示 PostgreSQL `tables_verified=true`，`/metrics` 可达；`TODO-P2-1E` authenticated `/api/dashboard/overview` 与 `/api/metrics/{video_id}` 均返回 200，post-revoke 返回 401；`TODO-P2-1G` 生产 `/api/dashboard/overview?days=7` 返回 `data/videos/scenarios/platforms` 且缺失字段为 `0`，浏览器上下文 authenticated GET 通过、`non_get_count=0`；`TODO-P2-1I` `/dashboard=200`，authenticated 页面进入非错误空数据态，生产 dashboard `days=7` GET 计数 `1`、`non_get_count=0`、临时 key 已撤销并 post-revoke `401`；但 `MetricsPoller` 未注册 startup scheduler，TikTok/Shopify fetcher 仍为 stub，未调用生产 `/metrics/pull`，webhook.site 与真实 platform metrics 数据流未验证 |
