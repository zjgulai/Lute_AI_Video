...
2026-06-19 P1 S1 bounded media trace_id 持久化收口：`TODO-P1-1-prep` 已通过 PR `#35` 合并 S1 bounded media contract 与 readiness/live spec；随后按授权执行一次 `TODO-P1-1` S1 bounded media live smoke，范围为 1 次 `/api/scenario/s1` submit、`provider_max_retries=0`、`provider_job_caps={image:1, video:1}`、`artifact_disposition=pending_review`。本次 live smoke 未进入 poyo image 或 Seedance：DeepSeek 文本阶段返回 `402 Payment Required` 共 `3` 次，Playwright 因 `bounded media pilot must generate at least one clip` 失败，`clip_paths=[]`，poyo submit、Seedance、TTS、thumbnail、assemble、media_quality_audit、gate candidate、final_work、publish、delivery 与 approved brand token 均为 `0`。同轮暴露出 PG 持久化缺陷：`pipeline_states.trace_id` 期望 `str`，但 S1 state 中出现 `int` trace_id，导致 `expected str, got int`，文件系统 state 已写入但 PG status readback 滞后。修复已通过 PR `#36` 合并到 `main`，merge commit 为 `43a4ec863469fc5898ace553436da3ce02c46d79`，main 上 `CI` 与 `Deploy to GitHub Pages` 均为 `success`；随后 `TODO-P1-1R-sync-prep` 已按授权只同步 `src/pipeline/state_manager.py` 到生产，远端备份目录为 `/opt/ai-video/backups/todo-p1-1r-trace-id-persistence-sync-prep-20260619T060806Z`，只重启 `ai_video_backend`。生产 `/api/health=200` / `status=ok`，本地/生产主机/容器 hash 均为 `0ae30409cd32ad74dd280e0cf4c10bc96f0973d4c7d553bd59a310a40a61b0cf`，容器 introspection 证明 `_repository_payload({"trace_id": 55663019})` 输出 `"55663019"` 且类型为 `str`；370 秒 no-submit backend log gate forbidden total 为 `0`。证据文件：`tmp/debug/todo-p1-1-corrected-failure-summary-20260619052651.json`、`tmp/debug/todo-p1-1-live-playwright-20260619052651.log`、`tmp/debug/todo-p1-1-live-backend-20260619052651.log`、`tmp/debug/todo-p1-1r-trace-id-persistence-sync-prep-summary-20260619T060806Z.json`、`tmp/debug/todo-p1-1r-trace-id-persistence-sync-prep-backend-log-window-20260619T060806Z.log`。当前 `TODO-P1-1` 不再是未授权状态，而是 `blocked_by_deepseek_balance_after_trace_id_sync`；下一次 live smoke 必须先确认 DeepSeek 余额恢复，再重新授权单 spec、单 submit、预算、job cap 与止损，不得直接重跑。
...
|---|---|---|---|---|
| TODO-P1-1 | S1 bounded media single-submit smoke | blocked_by_deepseek_balance_after_trace_id_sync | 已完成 contract/readiness 与 trace_id PG 持久化修复；下一次 live smoke 需先确认 DeepSeek 余额恢复，并重新限定 submit=1、image/video job cap、预算、artifact_disposition=pending_review、retry=0 | 上次 live smoke 未进入 poyo/Seedance，因 DeepSeek `402 Payment Required` 失败；trace_id 修复已合并并生产同步通过；重跑时仍只允许 tenant-scoped `pending_review` 或 quarantine，`final_work=0`，无 publish/delivery/token write |
