...
仓库侧修复已通过 PR `#68` 的 7 项 GitHub 检查，并合并为 `c05ac4ecb41034bc9d2b45cdfb974a9cb3a243e8`。随后经 L4 授权已完成备份工具最小同步、root cron 原子迁移和首个 `2026-07-10_161607` 数据库/媒体备份；全部 12 张表共 185 行、1617 个媒体文件及逐文件 checksum 均通过，生产应用容器 restart 仍为 0。隔离恢复进一步发现：远端旧 `001_init.sql` 只有 7 张表，实际生产数据库为 PostgreSQL 18.4 且历史 7 表类型与当前 fresh init 不同，因此该首备份虽数据/媒体完整，却不包含可独立重建生产 schema 的工件。`codex/l4-schema-backup-restore-20260710` 已补充同主版本 `pg_schema.dump`、对象清单、数据事务前后列签名、RepoDigest pin、checksum 真实绑定、空目标校验、临时 `0600` passfile、schema 单事务、one-shot 数据恢复/逐表核对和 restore marker；retention 在没有 verified 恢复点时不删除，并始终保留最新 verified 目录。候选脚本已在生产主机完成真实隔离恢复：`/opt/ai-video-backups/restore-drills/20260710T091854Z-candidate/summary.json` 记录 12 表逐表相等、总计 185 行，生产数据库未写、应用未重启、provider 未调用。该证据证明候选恢复链可行，但不是实际定时备份脚本发布的新数据库/媒体恢复点；在修复合并、最小同步、生成新版完整备份并由同一备份写入 marker 前，部署判定仍为 `blocked_by_schema_restore_and_secret_rotation`。生产 cron/备份目录和 `postgres:18` 客户端镜像已发生授权运维变更，但生产应用未部署或重启，`provider_call=false`，`no publish/delivery`。剩余 P2 基础设施缺口：manifest/checksum 尚未由外部不可变存储或独立签名密钥认证，当前证据不覆盖 root 级同时篡改 archive 与 manifest 的威胁模型。

## 2026-07-10 L4 备份与密钥门禁更新

PR `#70` 已合并 schema-backed backup/restore remediation。生产只读核验显示：备份 `2026-07-10_112359` 为 `complete`，包含 PostgreSQL 18 schema archive/list/signature、12 张表共 185 行、1617 个媒体文件，且 `restore_verified=passed`、无 partial 残留。脱敏 tenant metadata 显示 incident replacement key active、受影响 legacy key revoked；未读取 key hash 或历史明文。生产应用容器未重启，`provider_call=false`，`no publish/delivery`。

剩余部署前动作是合并当前 Lighthouse rsync sidecar exclude 变更（保护 `deploy/lighthouse/plugin-hub.htpasswd`），然后从 clean `main` 重跑 `DRY_RUN=1`，确认无意外删除项。正式生产 deploy 尚未执行。
...
该 P1/P2 收口阶段的证据上限为 `L3-production-read-only`：本地渲染镜像构建和健康检查通过；生产 12 个页面为 200，`/api/health` 显示 PostgreSQL、Remotion、ffmpeg、Chromium、yt-dlp、Whisper、CLIP 健康，四个容器运行且 restart=0，nginx 配置检查通过，该阶段只读窗口内 5xx、生成 submit、provider submit、publish、delivery acceptance 均为 0。当时 production unchanged，`provider_call=false`，`no publish/delivery`；后续备份运维变更以上一节为准。
...
2026-06-18 ToolBox 生产前端同步与只读验证收口：`L5-UI-5` 已按 frontend-only 范围同步已合并 `main` 的 `web/src/components/Nav.tsx`、`web/src/components/api.ts` 与 `web/src/components/QuickTemplate.tsx` 到生产 web 目录。生产 build 通过；只重启 `ai_video_frontend`，未重启 backend、rendering 或执行全量部署；`/api/health` 返回 `200`。桌面与 390px mobile 只读浏览验证覆盖 `/toolbox`、`/toolbox/product-image`、`/toolbox/six-view`、`/toolbox/ecommerce-visual`、`/toolbox/digital-human`、`/toolbox/storyboard` 与 `/toolbox/avatar`，有效页均返回 `200`，`/toolbox/avatar` 保持 `Unknown tool` fail-closed，未认证浏览器不再被只读背景 `401` 踢回 `/?session_expired=1`。浏览器审计显示 `non_get_count=0`、`forbidden_request_count=0`、`mobile390 overflow=0`；backend 日志窗口无 provider、scenario submit、Fast Mode submit、publish/delivery 匹配。证据文件：`tmp/debug/l5-ui5-production-sync-summary-20260618T150654Z.json`、`tmp/debug/l5-ui5-production-toolbox-readonly/summary.json`、`tmp/debug/l5-ui5-backend-log-window.log`。边界：本次只证明 ToolBox 前端生产同步与 read-only 路由/认证边界，不代表 provider 调用、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write 已执行。
...
2026-06-19 S2 full-media segmented readiness 收口：`TODO-P1-5-prep` 已新增 no-provider readiness guard `web/e2e/production/scenario-s2-full-media-segmented-readiness.prod.spec.ts`，并在 E2E 计划中明确后续必须拆成 TTS、thumbnail、assemble、audit 分段授权。随后 `TODO-P1-5A` 已完成本地 S2 segmented stop-point contract：`S2BrandCampaignRequest`、`/scenario/s2` router 与 `src/pipeline/s2_brand_pipeline_v2.py` 显式支持 `media_stop_step`，覆盖 `seedance_clips`、`tts_audio`、`thumbnail_prompts`、`thumbnail_images`、`assemble_final`、`audit` 六个 stop point；`tests/test_s2_e2e.py` 以 no-provider fake runner 证明每个 stop point 不越界，S1 复用 step 已补齐 TTS/thumbnail 的 pending_review output_dir、retry=0 和 job cap 透传。验证结果：`tests/test_s2_e2e.py` 为 `35 passed`，S2 segmented readiness Playwright 为 `1 passed`。边界：本轮不触碰生产、不调用 provider、不执行 `/api/scenario/s2` submit；S2 TTS、thumbnail、assemble、media_quality_audit、full media、publish、delivery acceptance 或 approved brand token write 仍未执行。

2026-06-21 P1-5C-1R S2 segmented `tts_audio` live segment 收口：首次 `P1-5C-1` 已执行 1 次 `/api/scenario/s2` submit 并产出 1 个 TTS 音频，但 DeepSeek 文本阶段出现 `402 Payment Required / Insufficient Balance`，按严格门禁不通过，且没有第二次 submit。DeepSeek 充值后，用户重新授权执行 `P1-5C-1R`；本轮只运行 S2 segmented live smoke，`media_stop_step=tts_audio`，只允许 DeepSeek 文本与 TTS，禁止 poyo image、Seedance、thumbnail、assemble、media_quality_audit、gate、publish、delivery 与 approved brand token。结果为 `passed`：仅 1 次 `/api/scenario/s2` submit，DeepSeek HTTP `200` 计数为 `2`，SiliconFlow/CosyVoice TTS HTTP `200` 计数为 `1`，`provider_max_retries=0`，临时 production key 已撤销且 post-revoke protected check 返回 `401`。生产状态与文件系统 readback 显示 `bounded_media_stop_step=tts_audio`、`provider_job_caps={tts:1}`、`tts_audio=done`，音频 `/app/output/tenants/momcozy-marketing/pending_review/p1_s2_segmented_tts_r_20260620T181821Z/audio/cosyvoice_en_38d3eea0.mp3` 位于 tenant-scoped `pending_review`，`quarantine` 不存在，`final_work_match_count=0`；`keyframe_images`、`seedance_clips`、`thumbnail_prompts`、`thumbnail_images`、`assemble_final` 与 `audit` 均仍为 `pending`。证据文件：`tmp/debug/todo-p1-5c-final-summary-20260620T181821Z.json`、`tmp/debug/todo-p1-5c-log-gate-20260620T181821Z.json`、`tmp/debug/todo-p1-5c-status-20260620T181821Z.json`、`tmp/debug/todo-p1-5c-filesystem-20260620T181821Z.json`、`tmp/debug/todo-p1-5c-backend-log-20260620T181821Z.log`。证据等级为 `L4-authorized-live-provider-tts-only`；本轮只证明 S2 分段 TTS 到 `tts_audio`，不代表 S2 thumbnail、assemble、media_quality_audit、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-21 P1-5C-2 S2 segmented `thumbnail_images` live segment 收口：用户授权执行单 spec `web/e2e/production/scenario-s2-thumbnail-segment-live.prod.spec.ts`，`media_stop_step=thumbnail_images`，只允许 DeepSeek 文本与 thumbnail image generation，禁止 keyframe image、Seedance、TTS、assemble、media_quality_audit、gate、publish、delivery 与 approved brand token。结果按 provider smoke 口径为 `passed`：Playwright return code 为 `0`，仅 1 次 `/api/scenario/s2` submit，poyo `gpt-image-2` submit 计数为 `1`，`provider_max_retries=0`；生产状态 readback 显示 `strategy`、`scripts`、`thumbnail_prompts`、`thumbnail_images` 为 `done`，`compliance`、`storyboards`、`continuity_storyboard_grid`、`keyframe_images`、`video_prompts`、`seedance_clips`、`tts_audio`、`assemble_final` 与 `audit` 均仍为 `pending`，`pipeline_degraded=false`。文件系统 readback 显示 1 张 thumbnail 位于 `/app/output/tenants/momcozy-marketing/pending_review/p1_s2_segmented_thumbnail_20260621T051833Z/thumbnails/`，`video_file_count=0`、`audio_file_count=0`、`matching_final_work_count=0`。原始一次性收口脚本因日志 gate 将同一次 POST 的 uvicorn 与 middleware 日志重复计数、且 key revoke helper 使用过期 import 而判 `failed`；已生成 corrected closeout summary，按 `key_id` 在生产 DB 撤销临时 key，`revoked=true`，本地明文 key 文件与 raw log 已删除。证据文件：`tmp/debug/todo-p1-5c-thumbnail-closeout-20260621T051833Z.json`、`tmp/debug/todo-p1-5c-thumbnail-final-summary-20260621T051833Z.json`、`tmp/debug/todo-p1-5c-thumbnail-status-20260621T051833Z.json`、`tmp/debug/todo-p1-5c-thumbnail-filesystem-20260621T051833Z.json`、`tmp/debug/todo-p1-5c-thumbnail-backend-20260621T051833Z.log`。证据等级为 `L4-authorized-live-provider-thumbnail-only`；本轮只证明 S2 segmented thumbnail 到 `thumbnail_images`，不代表 S2 assemble、media_quality_audit、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-21 TODO-P1-5D-prep S2 segmented `assemble_final` refs-only 合同准备：为避免 assemble segment 隐式串回 DeepSeek、poyo image、Seedance 或 TTS，上游合同已收窄为 `media_stop_step=assemble_final` 必须显式提供 `media_refs`。`S2BrandCampaignRequest` 新增 `media_refs`，`/scenario/s2` router 显式透传，`src/pipeline/s2_brand_pipeline_v2.py` 将 `assemble_final` 分段 step order 收窄为仅执行 `assemble_final`，`provider_job_caps={}`，并在运行前注入已存在的 tenant-scoped `pending_review`/`quarantine` clip、audio、thumbnail refs。refs 指向 `final_work`、`renders`、`fast_mode` 或 `gpt_images` 会 fail-closed；assemble 输出会搬移到 `tenants/{tenant}/{disposition}/{label}/assemble/`，对外只暴露 `intermediate_video_path` / `intermediate_render_json_path`，`final_video_path` 与 `render_json_path` 仍为空，`delivery_accepted=false`、`publish_allowed=false`、`approved_brand_token_write=false`。新增 readiness/live spec：`web/e2e/production/scenario-s2-assemble-segment-readiness.prod.spec.ts` 与 `web/e2e/production/scenario-s2-assemble-segment-live.prod.spec.ts`，后者仅在未来显式 `RUN_TOKEN_SMOKE=1`、单文件选择且提供 refs env 时才允许 submit。验证结果：`tests/test_s2_e2e.py` 为 `37 passed`；ruff 目标检查通过；readiness Playwright `1 passed`；live spec 在 `RUN_TOKEN_SMOKE=0` 下被默认排除，在 `RUN_TOKEN_SMOKE=1 --list` 下只枚举 1 个 `@token-smoke` 测试；`prodE2eTokenGuard` 为 `7 passed`。边界：本轮不触碰生产、不调用 provider、不执行 `/api/scenario/s2` submit、不执行真实 assemble live、不发布、不做 delivery acceptance、不写 approved brand token。

2026-06-21 P1-5D S2 segmented `assemble_final` refs-only live 收口：用户随后授权生产环境 `P1-5D S2 segmented assemble refs-only single-submit smoke`。本轮只运行 `web/e2e/production/scenario-s2-assemble-segment-live.prod.spec.ts`，`RUN_TOKEN_SMOKE=1`、`PLAYWRIGHT_MAX_SUBMIT_COUNT=1`、`PLAYWRIGHT_PROVIDER_MAX_RETRIES=0`、`PLAYWRIGHT_ARTIFACT_DISPOSITION=pending_review`、`PLAYWRIGHT_S2_SEGMENTED_MEDIA_STOP_STEP=assemble_final`，并显式传入既有 pending_review clip/audio/thumbnail refs。结果为 `passed`：run label 为 `p1_s2_segmented_assemble_20260621T153042Z`，Playwright `1 passed`，仅 1 次 `/api/scenario/s2` submit，provider HTTP / DeepSeek / poyo / Seedance provider / TTS / thumbnail generation / keyframe generation / media_quality_audit / gate candidate 计数均为 `0`，只执行允许的 local Remotion assemble `1` 次。文件系统 readback 显示 `pending_review` 下生成 1 个 mp4 与 1 个 render input JSON，`matching_final_work_count=0`；临时 production key 已撤销，post-revoke protected check 返回 `401`，本地明文 key 文件已删除。原始 runner summary 因二次判定器读取 status 顶层字段过严给出 `non_passing_decision`；已按 Playwright 断言、status `steps.assemble_final.output.refs_only=true`、filesystem readback 与 log gate 生成 corrected adjudication。证据文件：`tmp/debug/p1-5d-assemble-live-adjudication-20260621T153042Z.json`、`tmp/debug/p1-5d-assemble-live-summary-20260621T153042Z.json`、`tmp/debug/p1-5d-assemble-playwright-20260621T153042Z.log`、`tmp/debug/p1-5d-assemble-backend-20260621T153042Z.log`、`tmp/debug/p1-5d-assemble-status-20260621T153042Z.json`、`tmp/debug/p1-5d-assemble-filesystem-20260621T153042Z.json`。证据等级为 `L4-authorized-live-local-assemble-only`；本轮只证明 S2 segmented refs-only local assemble 到 `assemble_final`，仍不代表 S2 media_quality_audit、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-22 TODO-P1-5E-prep S2 segmented `audit` refs-only 合同准备：为避免 media_quality_audit segment 为补齐上游状态而隐式触发 DeepSeek、poyo image、Seedance、TTS、thumbnail 或 assemble，本轮将 `S2_SEGMENTED_MEDIA_STEP_ORDERS["audit"]` 收窄为仅执行 `audit`，`provider_job_caps={}`，并要求 `media_stop_step=audit` 显式提供 tenant-scoped `pending_review`/`quarantine` media refs：clip、audio、thumbnail、assemble video，可选 render JSON / lyrics / thumbnail prompts / scripts / storyboards / continuity grid。refs 指向 `final_work`、`renders`、`fast_mode` 或 `gpt_images` 会 fail-closed；state/config 标记 `refs_only_media_audit=true`，`final_video_path` 与 `render_json_path` 仍为空，`delivery_accepted=false`、`publish_allowed=false`、`approved_brand_token_write=false`。新增 readiness/live spec：`web/e2e/production/scenario-s2-audit-segment-readiness.prod.spec.ts` 与 `web/e2e/production/scenario-s2-audit-segment-live.prod.spec.ts`，后者仅在未来显式 `RUN_TOKEN_SMOKE=1`、单文件选择且提供 refs env 时才允许 submit。验证结果：`tests/test_s2_e2e.py` 为 `39 passed`；ruff 目标检查通过；readiness Playwright `1 passed`；live spec 在 `RUN_TOKEN_SMOKE=1 --list` 下只枚举 1 个 `@token-smoke` 测试，`RUN_TOKEN_SMOKE=0 --list` 下默认排除；`prodE2eTokenGuard` 为 `7 passed`。边界：本轮不触碰生产、不调用 provider、不执行 `/api/scenario/s2` submit、不执行真实 audit live、不发布、不做 delivery acceptance、不写 approved brand token。

2026-06-22 P1-5E S2 segmented `audit` refs-only live smoke 与合同漂移收口：`P1-5E-sync-prep` 已通过 OrcaTerm console path 将 `src/routers/_state.py`、`src/routers/scenario.py`、`src/pipeline/s2_brand_pipeline_v2.py` 同步到生产并完成 hash verify、import/introspection smoke、`/api/health` 与 6 分钟 no-submit/provider log gate；证据为 `tmp/debug/p1-5e-sync-prep-summary-20260622T071200Z.json`。随后用户授权执行 `P1-5E` live smoke，run label 为 `p1_s2_segmented_audit_20260622T074557Z`。生产状态 readback 显示 pipeline `status=completed`、`audit=done`、`pipeline_degraded=false`、`audit_report_present=true`、`audit_overall_status=FAIL`，该 FAIL 是 media quality audit 对既有短 refs 的质量判定，不是 provider 或 pipeline 执行失败；`keyframe_images` 与 `video_prompts` 保持 `pending`，`continuity_storyboard_grid` 被 refs-only input 注入为 `done` 且 `output.status=refs_only`。backend 日志只有 1 次实际 `POST /scenario/s2`；旧 summary 的 `scenario_submit_count=2` 来自 middleware 与 uvicorn 同一次 POST 的重复日志行计数。provider HTTP、DeepSeek、poyo、Seedance、TTS、thumbnail generation、keyframe generation、gate、final_work、publish/delivery 计数均为 `0`，临时 production key 已撤销且 post-revoke protected check 返回 `401`。当次 Playwright 返回 nonzero，原因是旧 spec 仍期望 `continuity_storyboard_grid` 为 `pending`；随后 PR `#44`（merge commit `b30afc9`）已将本地 Python/Playwright 合同对齐为接受 refs-only seeded step，PR 与 main checks 均为 success。边界：本轮不代表 P1-5E 当次 Playwright 绿灯重跑；如需以修正后的 live spec 获取 green Playwright 证据，仍需重新授权一次单 submit。

2026-06-23 P1-6A-R2S S1 gate corrected live smoke 收口：用户在 DeepSeek 充值后授权重跑单标题 `web/e2e/production/s1-gate.prod.spec.ts` 的 `step 2: gate exists after scripts and exposes 3 candidates @token-smoke`。本轮 `RUN_TOKEN_SMOKE=1`、`PLAYWRIGHT_PROD_WORKERS=1`、`PLAYWRIGHT_MAX_SUBMIT_COUNT=1`、`PLAYWRIGHT_EXPLICIT_SPEC=s1-gate.prod.spec.ts`、`PLAYWRIGHT_S1_GATE_GENERATE_TIMEOUT_MS=120000`、`PLAYWRIGHT_S1_GATE_TEST_TIMEOUT_MS=180000`、`--retries=0`。结果为 `passed`：Playwright `1 passed (1.3m)`；应用日志中 `/scenario/s1/start`、`/scenario/s1/step/strategy`、`/scenario/s1/step/scripts`、`/scenario/s1/gate/{label}/gate_1_script/generate` 各 1 次；DeepSeek HTTP `200 OK` 计数为 `8`，`402 Payment Required` 计数为 `0`；poyo、Seedance、TTS、thumbnail、assemble、media_quality_audit、final_work、publish、delivery 与 approved brand token 禁止项均为 `0`。临时 production key 已撤销，post-revoke protected check 返回 `401`，本地明文 key 文件已删除。证据文件：`tmp/debug/p1-6a-r2s-final-adjudication-20260623T094047Z.json`、`tmp/debug/p1-6a-r2-summary-20260623T094047Z.json`、`tmp/debug/p1-6a-r2-playwright-20260623T094047Z.log`、`tmp/debug/p1-6a-r2-backend-20260623T094047Z.log`。证据等级为 `L4-authorized-live-s1-gate-candidate-only`；本轮只证明 S1 gate candidate generation 在 scripts 后可产生 3 个 candidates，不代表 gate approve/regenerate、S1 media generation、S2-S5、Fast Mode、完整 token suite、publish、delivery acceptance 或 approved brand token write 已执行。
...
2026-06-19 P2 多租户/API key 隔离生产只读探针：`TODO-P2-2B` 已按授权创建 2 个 2 小时临时 non-demo production key（tenant=`momcozy-marketing` 与 `default`），只执行 GET 后均已撤销，post-revoke `/api/dashboard/overview?days=7` 均返回 `401`。生产 `/api/health` 与 `/api/dashboard/overview?days=7` 对两个 key 均返回 `200`；S2 state 隔离通过：`momcozy-marketing` key 可读 `l4d5s_s2_bounded_keyframe_20260613132543` 且读取 `default` label `s2_momcozy_1781172006` 返回 `404`，`default` key 可读历史 `default` label 且读取 `momcozy-marketing` label 返回 `404`，不存在 label 对两者均返回 `404`。Portfolio 只读过滤结果为：普通 tenant `momcozy-marketing` 仅看到 `momcozy-marketing` pending_review；`default` key 作为 legacy/global compatibility 可见 `default` 与 `momcozy-marketing` pending_review，该行为按兼容边界记录，不误判为普通租户串租。backend 日志窗口 provider、scenario submit、Fast Mode submit、publish/delivery、output write 与 mutating HTTP 计数均为 `0`。sanitized summary 为 `tmp/debug/todo-p2-2b-multi-tenant-readonly-summary-20260618161535.json`，原始未修正判定保留为 `tmp/debug/todo-p2-2b-multi-tenant-readonly-summary-20260618161535.raw.json`，backend log 为 `tmp/debug/todo-p2-2b-multi-tenant-readonly-backend-log-window-20260618161535.log`。证据等级为 `L3-production-read-only`；本轮不代表真实并发 soak、Locust 压测、provider、submit、publish、delivery acceptance 或 approved brand token 已执行。
...
2026-06-18 P2 Quality ML 生产容器 no-provider smoke：`TODO-P2-3C` 已按授权只在 `ai_video_backend` 容器内执行一次 Python one-off，不同步代码、不重启服务、不下载 CLIP/transformers 权重、不写 output volume。生产 `/api/health` 返回 `200` / `status=ok`，`media_tools.ytdlp_available`、`media_tools.whisper_available`、`media_tools.clip_available` 均存在且为 bool。容器 import smoke 覆盖 `src.quality.clip_alignment`、`src.quality.nr_quality`、`src.quality.scene_analysis`、`src.quality.face_consistency`、`src.quality.safe_zone`、`src.skills.media_quality_audit`、`src.routers.health`，结果全部成功。CLIP fallback contract 通过：通过 monkeypatch `ClipAligner._load` 为 unavailable 验证 `check_batch()` 返回 `score=None`、`aligned=None`、`reason=CLIP unavailable...`，并确认本测试未触发权重下载。backend 日志窗口 provider、scenario submit、Fast Mode submit、publish/delivery 禁止项匹配为 `0`。证据文件：`tmp/debug/todo-p2-3c-quality-ml-production-container-no-provider-summary-202606181548.json`、`tmp/debug/todo-p2-3c-container-smoke.json`、`tmp/debug/todo-p2-3c-health.txt`、`tmp/debug/todo-p2-3c-backend-log-window.log`。证据等级为 `L3-production-container-read-only-no-provider`；本轮只证明生产容器 no-provider 可用性，不代表真实 ML 权重推理质量、CLIP 语义评分质量或任何 provider / submit / publish 链路已执行。
...
2026-06-17 P2 dashboard contract 本地修复：`TODO-P2-1F` 已定位并修复 `/dashboard/overview` 与前端 `PerformanceDashboard` 的响应契约漂移。生产 readback 只能证明 authenticated GET 返回 200，不能证明前端真数据渲染可用；原因是后端此前返回 `{scenario, platform, days, data}`，而前端期望 `{videos, scenarios, platforms}`。本地修复保留旧 `data` 字段，并新增前端所需 `videos/scenarios/platforms` 聚合结构；`tests/test_metrics_dashboard.py` 新增空数据合同与真实 row 转换测试。验证：`ruff` 目标检查通过，`tests/test_metrics_dashboard.py tests/test_video_metrics_integration.py tests/test_metrics_poller.py` 结果 `15 passed`。证据等级仅为 `L2-fixture-or-dry-run`；本轮未同步生产、未创建 production key、未执行 `/metrics/pull`、未调用真实 TikTok/Shopify metrics connector、未配置 webhook 接收端、未做前端生产页面真数据 readback、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-17 P2 dashboard contract 生产同步与 authenticated browser readback：`TODO-P2-1G` 已按授权只同步 `src/routers/metrics.py` 到生产，远端原文件备份到 `/opt/ai-video/backups/todo-p2-1g-dashboard-contract-20260617150500/`，重启 `ai_video_backend` 后 `/api/health` 返回 `200` 且 Docker health 为 `healthy`。hash verify 通过：本地、远端 `/opt/ai-video/src/routers/metrics.py` 与容器 `/app/src/routers/metrics.py` 均为 `2b84461789b5b9a066df85f828f462ab5e550d89397ed0a2b1183495ba53e0c5`。随后创建 1 个 2 小时临时 non-demo production key（tenant=`momcozy-marketing`，masked=`todo...uC38`，key_id=`c75c63a4-0c0d-49eb-b092-a605254de753`），只执行 GET：`/api/health=200`、`/api/dashboard/overview?days=7=200`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent=200`，dashboard response 包含 `data/videos/scenarios/platforms` 且缺失字段为 `0`。浏览器只读 readback 在生产页面上下文中验证 authenticated storage 与同三类 GET 均通过，`non_get_count=0`，dashboard error 文案不可见；但 `PerformanceDashboard` 只挂载在一次性结果页的内存 `oneshotResult` + `data` tab 后，生产 read-only 导航无法在不 submit 的情况下自然创建该状态，因此本轮不能声称完整组件自然渲染已验收。验证后已撤销该 key，post-revoke dashboard GET 返回 `401`。sanitized summary 为 `tmp/debug/todo-p2-1g-dashboard-contract-production-readback-20260617150500.json`。本轮未执行全量生产部署、provider 调用、`/api/scenario/*` submit、Fast Mode submit、`/metrics/pull`、webhook 外发、publish、delivery acceptance 或 approved brand token write。
...
2026-06-17 P2 metrics pull fail-closed guard 主线合并与生产同步：`TODO-P2-1J` 已通过 PR `#25` 合并进 `main`，merge commit 为 `646b621e382dcdef5178260aaccc86ef85175851`。代码增加 `METRICS_PULL_ENABLED` 显式开关，使 `POST /metrics/pull` 默认返回 `403 Metrics pull is disabled`，避免真实 platform fetcher / scheduler 尚未闭环时被任意生产 key 触发；只有显式启用后才会调用 `MetricsPoller.pull_all()`。新增测试覆盖默认禁用不调用 poller、显式启用才调用 poller。合并后本地 `main`、`origin/main` 与 GitHub `main` SHA 已对齐，main 上 `CI` 与 `Deploy to GitHub Pages` checks 均为 `success`；本地 no-provider smoke `DATABASE_URL= .venv/bin/pytest tests/test_metrics_dashboard.py tests/test_metrics_poller.py -q` 结果 `8 passed`。随后 `TODO-P2-1K` 已按授权只同步 `src/config.py` 与 `src/routers/metrics.py` 到生产，远端原文件备份到 `/opt/ai-video/backups/todo-p2-1k-metrics-pull-guard-20260617231650/`，只重启 `ai_video_backend`，未执行全量部署。hash verify 通过：`src/config.py` 本地=远端=容器 `a53bf040dbc9d8918f0ff0147a15e605c6feb1ef361f3ad52108c3bae7352187`，`src/routers/metrics.py` 本地=远端=容器 `a7c5aacec699136dbcd5c3490cb77f3ea70833ae47af1b48de894098cdbb7b85`；容器 import smoke 显示 `import_ok=true`、`metrics_pull_enabled=false`。生产 `/api/health` 返回 `status=ok`，backend 容器为 `running/healthy`。随后创建 1 个 2 小时临时 non-demo production key（tenant=`momcozy-marketing`，masked=`todo...sTUA`），GET `/api/health=200`，POST `/api/metrics/pull=403 Metrics pull is disabled`，撤销后 protected GET `/api/dashboard/overview?days=7=401`；sanitized summary 为 `tmp/debug/todo-p2-1k-metrics-pull-guard-verification-20260617151912.json`。refined backend log gate 显示 provider、scenario submit、Fast Mode submit、真实 metrics pull、webhook 外发、publish/delivery/token 写入均为 `0`；允许项仅为启动时本地 webhook listener subscription 与本轮 `metrics_pull_403` 请求。本轮未启用 `METRICS_PULL_ENABLED`、未调用 `MetricsPoller.pull_all()`、未调用真实 TikTok/Shopify platform metrics、未配置 webhook 接收端、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 metrics / webhook / analytics 本地真实事件链路合同继续收口：`TODO-P2-1L1/L2/L3` 已完成本地 no-provider 合同层。`L1/L2` 通过 PR `#26` 合并进 `main`，merge commit 为 `8ba43163c1a3b2eef126d00c34c98c520e283dfc`，`MetricsPoller` 已支持注入 fake platform fetcher，unknown platform skip、recent snapshot skip、SQLite timestamp string 解析、fake metrics -> repository -> dashboard readback 均有测试覆盖，目标测试 `DATABASE_URL= .venv/bin/pytest tests/test_metrics_poller.py -q` 结果 `6 passed`。本轮 `L3` 在本地分支完成 webhook receiver contract：`WebhookManager` 支持注入 `http_sender` fake receiver 与 `timeout_seconds`，`dispatch()` 返回包含 `event_type`、`event_id`、listener 计数、HTTP attempted/succeeded/failed 和逐 URL result 的 sanitized summary；fake receiver readback 可验证 envelope 的 `event_type/timestamp/event_id/data`，receiver failure 与 timeout 会进入 summary 且不阻塞其他 receiver。目标测试 `DATABASE_URL= .venv/bin/pytest tests/test_webhook_manager.py -q` 结果 `18 passed`；metrics / webhook / analytics no-provider 回归集 `DATABASE_URL= .venv/bin/pytest tests/test_metrics_dashboard.py tests/test_video_metrics_integration.py tests/test_metrics_repository.py tests/test_metrics_poller.py tests/test_webhook_manager.py tests/test_portfolio_mechanism.py tests/test_agents.py -q` 结果 `118 passed`。证据等级仍为 `L2-fixture-or-dry-run`；未启用 `METRICS_PULL_ENABLED`，未执行 `/api/metrics/pull`，未注册 startup scheduler，未调用 TikTok/Shopify 真实 metrics API，未配置 webhook.site 或真实外部接收端，未触发真实 webhook 外发，未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 metrics / webhook / analytics 生产只读回归：`TODO-P2-1L4` 已按授权创建 1 个 2 小时临时 non-demo production key（tenant=`momcozy-marketing`，masked=`p2_1...K9Fn`，key_id=`bf29b35a-9902-4f12-84bc-70fae4c1ba0b`），只执行 GET：`/api/health=200`、`/api/dashboard/overview?days=7=200`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent=200`，dashboard response 包含 `data/videos/scenarios/platforms`。验证后已撤销该 key，post-revoke protected GET 返回 `401`，本地明文 key 文件已删除。backend log gate 显示 provider、scenario submit、Fast Mode submit、metrics pull、webhook 外发、publish/delivery/approved brand token 和 mutating HTTP 计数均为 `0`。sanitized summary 为 `tmp/debug/todo-p2-1l4-production-readonly-regression-20260617172948.json`，backend log 为 `tmp/debug/todo-p2-1l4-backend-log-window-20260617172948.log`。证据等级为 `L3-production-read-only`；本轮未生产部署、未重启任何服务、未调用 `/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics API、未配置外部 webhook receiver、未触发真实 webhook 外发、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 external webhook 单事件 smoke 首次尝试：`TODO-P2-1L5` 已按授权创建 1 个临时 webhook.site receiver，并在 `ai_video_backend` 容器内执行 1 次 Python one-off；但 one-off 在触发 webhook 前失败，receiver readback 显示 `received_count_delta=0`、总请求数仍为 `0`，因此不得声明真实 webhook 外发已通过。失败根因为生产容器内 `WebhookManager.__init__()` 尚不支持本地 main 已具备的 `timeout_seconds` 参数，one-off 抛出 `TypeError: WebhookManager.__init__() got an unexpected keyword argument 'timeout_seconds'` 后退出，未执行第二次 dispatch。backend log gate 禁止项均为 `0`，未出现 provider、scenario submit、Fast Mode submit、metrics pull、publish/delivery。sanitized summary 为 `tmp/debug/todo-p2-1l5-external-webhook-single-event-smoke-20260618022925.json`，backend log 为 `tmp/debug/todo-p2-1l5-backend-log-window-20260618022925.log`。证据等级保持为 failed-before-send，不能升级为 `L4-authorized-live-webhook-only`；下一步需要另行授权 `P2-1L5R`，使用与生产构造函数兼容的 one-off（不传 `timeout_seconds`）或先同步 `src/tools/webhook_manager.py` 后再 retry。

2026-06-18 P2 external webhook 单事件 smoke retry：`TODO-P2-1L5R` 已按授权创建 1 个新的临时 webhook.site receiver（masked=`https://webhook.site/44f8...1800`），并在 `ai_video_backend` 容器内执行 1 次生产兼容 Python one-off（`WebhookManager()`，不传 `timeout_seconds`）。本轮只注册 `event_type=audit.completed`，只触发 1 次 synthetic payload（`trace_id/thread_id/checkpoint/score`），receiver readback 显示新增 POST 数为 `1`，method 为 `POST`，envelope 包含 `event_type=audit.completed`、`event_id`、`timestamp` 与 `data`，且 `data` 与 synthetic payload 精确匹配。backend log window 未捕获到新增输出，log gate 禁止项计数均为 `0`。sanitized summary 为 `tmp/debug/todo-p2-1l5r-external-webhook-single-event-smoke-20260618024059.json`，backend log 为 `tmp/debug/todo-p2-1l5r-backend-log-window-20260618024059.log`。证据等级为 `L4-authorized-live-webhook-only`；本轮未生产部署、未同步代码、未重启任何服务、未调用 provider、未执行 `/api/scenario/*` submit、Fast Mode submit、`/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics、未执行 `pipeline.completed` / publish 相关事件、未 publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 platform metrics pull readiness no-pull probe：`TODO-P2-1L6-prep` 已按授权完成本地静态检查、生产只读检查和 no-provider 测试，判定 `L6` 当前不 ready，状态为 `blocked_by_real_connector_no_active_post`。本地静态证据显示 `src/tasks/metrics_poller.py` 默认 TikTok / Shopify fetcher 仍是 stub，分别返回 `{}`，`src/connectors/` 当前没有被 `MetricsPoller` 使用的真实 platform metrics pull 方法；`POST /metrics/pull` 仍由 `METRICS_PULL_ENABLED` fail-closed 保护。生产只读 `/api/health` 返回 `200` / `status=ok`，`persistence.backend=postgresql` 且 `tables_verified=true`；容器只读 introspection 显示 `metrics_pull_enabled=false`；只输出 env 名称不输出 secret value，当前可见平台相关 env 名称包括 `TIKTOK_ACCESS_TOKEN`、`YOUTUBE_API_KEY`、`SHOPIFY_API_KEY`、`SHOPIFY_STORE_URL`，其中 Shopify 名称与 `src/config.py` 读取的 `SHOPIFY_ACCESS_TOKEN` 不一致。生产 `VideoMetricsRepository.get_active_posts()` 返回 `active_post_count=0`，因此没有 allowlisted single-post pilot 候选。目标测试 `DATABASE_URL= .venv/bin/pytest tests/test_metrics_poller.py tests/test_metrics_dashboard.py -q` 结果 `12 passed`。sanitized summary 为 `tmp/debug/todo-p2-1l6-prep-platform-metrics-pull-readiness-20260618105143.json`。本轮未生产部署、未同步代码、未重启任何服务、未调用 `/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics API、未 webhook 外发、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 real platform metrics connector contract：`TODO-P2-1L6A` 已在本地分支完成最小 contract 修复并保持 no-provider。`MetricsPoller` 新增 `PlatformMetricsError` 分类：`auth`、`rate_limit`、`not_found`、`schema_drift`、`transient`、`not_implemented`；默认 TikTok / Shopify fetcher 不再返回空 dict 伪装成功，而是通过 connector `fetch_metrics()` fail-closed，空 payload 或无 recognized metric keys 会被判为 `schema_drift` 且不保存。`MetricsPoller.dry_run_due_posts()` 可只读检查 active posts / due posts / allowlisted candidates，不调用 platform fetcher、不写 DB。`TikTokConnector.fetch_metrics()` 与 `ShopifyConnector.fetch_metrics()` 已显式声明但仍抛 `NotImplementedError`，表示真实平台 metrics API mapping 尚未实现；Shopify 配置已把 `SHOPIFY_ACCESS_TOKEN` 回落到 legacy `SHOPIFY_API_KEY`，并显式保留 `SHOPIFY_STORE_URL`。目标验证：`ruff` 目标文件通过，`DATABASE_URL= .venv/bin/pytest tests/test_metrics_poller.py tests/test_metrics_dashboard.py -q` 结果 `19 passed`。生产只读复核显示 `/api/health=200`、`metrics_pull_enabled=false`、平台 env 名称只输出名称不展示 secret value，`active_post_count=0`。sanitized summary 为 `tmp/debug/todo-p2-1l6a-platform-metrics-connector-contract-20260618111500.json`。因此 `P2-1L6B` live pull 仍不 ready，当前阻塞为真实 TikTok/Shopify metrics API mapping 未实现且没有 allowlisted active post 候选；本轮未生产部署、未同步代码、未重启任何服务、未调用 `/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics API、未 webhook 外发、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 real platform metrics API mapping no-provider contract：`TODO-P2-1L6C` 已在本地分支实现 TikTok / Shopify `fetch_metrics()` 的真实 API mapping 代码路径，但测试只使用 mocked HTTP，未调用真实平台。TikTok mapping 使用官方 `/v2/video/query/`：`filters.video_ids` 请求体与 `view_count`、`like_count`、`comment_count`、`share_count` 字段，映射为 `views`、`likes`、`comments`、`shares`。Shopify mapping 使用 Admin GraphQL `shopifyqlQuery`，默认 query 为官方示例风格的单数据源 `FROM sales SHOW total_sales, orders SINCE -30d`；解析 `tableData.columns` / `tableData.rows` 与 `parseErrors`，并将 `total_sales`、`orders` 映射为 `revenue`、`orders/sales`，如自定义 query 返回 `sessions` 则额外映射 `views/cvr`。新增 mocked HTTP 合同测试覆盖 `auth`、`rate_limit`、`not_found`、`schema_drift`、`transient`、`not_implemented`，并确认 empty/unrecognized payload 不落库、dry-run readiness 不执行真实 pull。目标验证：`ruff` 目标文件通过，`DATABASE_URL= HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 .venv/bin/pytest tests/test_metrics_poller.py tests/test_metrics_dashboard.py -q` 结果 `29 passed`，`git diff --check` 无输出。证据等级为 `L2-local-mocked-http-contract`；`P2-1L6B` live pull 仍不 ready，阻塞项为生产 active post 候选仍为 `0`，且 ShopifyQL post-level 过滤维度尚未由本轮官方资料确认。未生产部署、未同步代码、未重启服务、未调用 `/api/metrics/pull`、未启用 `METRICS_PULL_ENABLED`、未调用真实 TikTok/Shopify metrics API、未 webhook 外发、未执行 provider、`/api/scenario/*` submit、Fast Mode submit、publish、delivery acceptance 或 approved brand token write。

2026-06-18 P2 active-post source contract：`TODO-P2-1L6E/L6F/L6G/L6H` 已继续收口到 no-pull readiness。`L6E` 已把 API mapping 相关已合并文件同步到生产并验证 `METRICS_PULL_ENABLED=false`、`dry_run_due_posts()` 可执行且不调用 platform fetcher；`L6F` 生产只读探查显示 `video_metrics.total=0`、`publish_logs.total=0`，状态为 `blocked_by_no_active_post_source`。`L6G` 本地合同修复明确 `VideoMetricsRepository.get_active_post_source_summary()` 中 `video_metrics` 是当前唯一 live pull active source，`publish_logs` 只作为只读 source evidence 且因缺少 `video_id/scenario/published_at` 不自动成为 pull candidate；manual allowlist 只过滤已存在 `video_metrics.post_id`，manual seed 被标记为需要另行 DB write 授权。随后 `L6H` 已把已合并 main 的 `src/storage/metrics_repository.py` 与 `src/tasks/metrics_poller.py` 同步到生产，远端备份目录为 `/opt/ai-video/backups/p2-1l6h-active-post-source-contract-20260618160644`，只重启 `ai_video_backend`；`/api/health=ok`，本地/生产主机/容器 hash verify 均通过，容器 import smoke 通过。生产 introspection 显示 `metrics_pull_enabled=false`，`dry_run_due_posts()` 返回 `source_contract`，readiness 仍为 `blocked_by_no_active_post_source`，`active_post_count=0`、`due_post_count=0`、`allowlisted_due_post_count=0`，`live_pull_ready=false`。证据 `tmp/debug/todo-p2-1l6h-active-post-source-production-sync-20260618160644.json` 记录 `evidence_grade=L4-authorized-production-sync-readonly-no-pull`；370 秒 backend log gate 显示 `/api/metrics/pull`、`MetricsPoller.pull_all`、真实 TikTok/Shopify pull、webhook 外发、provider、scenario/Fast submit、publish/delivery 均为 `0`。默认下一步仍不是 live pull；必须先取得 active post seed 或有效 `video_metrics` active source、single-post allowlist 和单独 `/api/metrics/pull` 授权。

2026-06-19 P1 S1 bounded media trace_id 持久化与 retry 收口：`TODO-P1-1-prep` 已通过 PR `#35` 合并 S1 bounded media contract 与 readiness/live spec。首次授权 live smoke 只执行 1 次 `/api/scenario/s1` submit，但 DeepSeek 文本阶段返回 `402 Payment Required` 共 `3` 次，未进入 poyo image 或 Seedance；同轮暴露出 PG 持久化缺陷：`pipeline_states.trace_id` 期望 `str`，但 S1 state 中出现 `int` trace_id。修复已通过 PR `#36` 合并到 `main`，merge commit 为 `43a4ec863469fc5898ace553436da3ce02c46d79`；随后 `TODO-P1-1R-sync-prep` 已按授权只同步 `src/pipeline/state_manager.py` 到生产，只重启 `ai_video_backend`，生产 `/api/health=200` / `status=ok`，本地/生产主机/容器 hash 均为 `0ae30409cd32ad74dd280e0cf4c10bc96f0973d4c7d553bd59a310a40a61b0cf`，容器 introspection 证明 `_repository_payload({"trace_id": 55663019})` 输出 `"55663019"` 且类型为 `str`，370 秒 no-submit backend log gate forbidden total 为 `0`。DeepSeek 充值后，用户授权执行 `TODO-P1-1R` S1 bounded media after-trace-id-sync provider smoke retry；本轮只运行 `web/e2e/production/scenario-s1-bounded-media-pilot-live.prod.spec.ts`，Playwright `1 passed (6.0m)`，临时 production key 已撤销且 post-revoke protected check 返回 `401`。生产 readback 显示 `pipeline_degraded=false`、`artifact_disposition=pending_review`、`provider_max_retries=0`、`provider_job_caps={image:1, video:1}`，DeepSeek HTTP `200` 计数为 `2`，poyo submit 总数为 `2` 且仅对应 1 个 image job + 1 个 Seedance video job，provider retry 与 provider error 均为 `0`。文件系统 readback 显示 1 个 keyframe、1 个 Seedance mp4 与 1 个 continuity frame 均位于 `/app/output/tenants/momcozy-marketing/pending_review/p1_s1_bounded_20260619070622/`，`final_work_match_count=0`；`seedance_clips=done`，`thumbnail_prompts`、`thumbnail_images`、`tts_audio`、`assemble_final` 与 `audit` 仍为 `pending`。原始二级 provider-boundary gate 因错误假设 `/api/scenario/s1/status` 顶层直接暴露 config 字段而误报失败，已用生产 state 文件与文件系统 readback 纠偏为 `passed_corrected`。证据文件：`tmp/debug/todo-p1-1-corrected-failure-summary-20260619052651.json`、`tmp/debug/todo-p1-1r-trace-id-persistence-sync-prep-summary-20260619T060806Z.json`、`tmp/debug/todo-p1-1r-corrected-live-summary-20260619070622.json`、`tmp/debug/todo-p1-1-production-state-filesystem-readback-20260619070622.json`、`tmp/debug/todo-p1-1-live-playwright-20260619070622.log`、`tmp/debug/todo-p1-1-live-backend-20260619070622.log`。证据等级为 `L4-authorized-live-provider-single-submit`；本轮只证明 S1 bounded media 到 `seedance_clips`，不代表 S1 full media/final assembly、S3/S4/S5 media、TTS、thumbnail、assemble、media quality audit、gate、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-19 P1 S3 bounded media provider smoke 与模型口径复核：`TODO-P1-2-prep` 已通过 PR `#37` 合并 S3 bounded media contract/readiness/live spec，并已按 `TODO-P1-2-sync-prep` 授权只同步 `src/routers/_state.py`、`src/routers/scenario.py` 与 `src/pipeline/s3_remix_pipeline.py` 到生产，只重启 `ai_video_backend`；生产 `/api/health=ok`，三文件本地/生产主机/容器 hash 一致，容器 introspection 通过，6 分钟 no-submit 日志窗口 forbidden total 为 `0`。随后用户授权执行 `TODO-P1-2` S3 bounded media single-submit provider smoke；本轮只运行 `web/e2e/production/scenario-s3-bounded-media-pilot-live.prod.spec.ts`，Playwright `1 passed (4.4m)`，`retries=0`、`workers=1`、临时 production key 已撤销且本地明文 key 文件已删除。生产 readback 显示 `pipeline_degraded=false`、`seedance_clips=done`，`thumbnail_prompts`、`thumbnail_images`、`tts_audio`、`assemble_final` 与 `audit` 均为 `pending`；provider boundary 显示仅 1 次 `/scenario/s3` submit，poyo image job=1、video job=1、provider retry=0、禁止项总数为 `0`。文件系统 readback 显示 1 个 keyframe、1 个 mp4 clip 与 1 个 continuity frame 均位于 `/app/output/tenants/momcozy-marketing/pending_review/p1_s3_bounded_20260619T084633Z/`，clip verification `all_ok=true`，未进入 `final_work`。随后 `P1-2Z` 只读复核确认：生产容器 `POYO_VIDEO_MODEL=happy-horse` 为全局 env 覆盖，`src/config.py` main/生产代码默认仍为 `seedance-2`；S3 路径显式通过 `ModelRouter.select_model("s3")` 选择 `kling-3.0/standard` 并传入 `seedance-video-generate-skill`，因此 P1-2 中日志观测到的 `kling-3.0/standard` 与 S3 model matrix 一致，不阻塞 P1-3/S4 prep。该复核同时记录一个后续 no-provider 配置治理项：在执行任何不显式传 model 的 generic/full-chain 视频 smoke 前，应先对齐生产全局 `POYO_VIDEO_MODEL` env 与 main 默认值。证据文件：`tmp/debug/todo-p1-2-sync-prep-summary-20260619T081727Z.json`、`tmp/debug/todo-p1-2-final-summary-20260619T084633Z.json`、`tmp/debug/todo-p1-2-backend-log-final-20260619T084633Z.log`、`tmp/debug/todo-p1-2-status-20260619T084633Z.json`、`tmp/debug/p1-2z-provider-video-model-drift-summary-20260619T090251Z.json`。证据等级为 `L4-authorized-live-provider-single-submit` + `L3-production-read-only-config-audit`；本轮只证明 S3 bounded media 到 `seedance_clips`，不代表 S3 full media/final assembly、S1/S2/S4/S5 media、TTS、thumbnail、assemble、media quality audit、gate、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-19 P1 S4 bounded media provider smoke 收口：`TODO-P1-3-prep` 已通过 PR `#38` 合并 S4 bounded media contract/readiness/live spec，并已按 `TODO-P1-3-sync-prep` 授权只同步 `src/routers/_state.py`、`src/routers/scenario.py` 与 `src/pipeline/s4_live_shoot_pipeline.py` 到生产，只重启 `ai_video_backend`；生产 `/api/health=ok`，三文件本地/生产主机/容器 hash 一致，容器 introspection 通过，6 分钟 no-submit 日志窗口 forbidden total 为 `0`。随后用户授权执行 `TODO-P1-3` S4 bounded media single-submit provider smoke；本轮只运行 `web/e2e/production/scenario-s4-bounded-media-pilot-live.prod.spec.ts`，Playwright `1 passed (3.9m)`，`retries=0`、`workers=1`、临时 production key 已撤销，post-revoke protected check 返回 `401`，本地明文 key 文件已删除。生产 readback 显示 `pipeline_degraded=false`、`seedance_clips=done`，`thumbnails`、`tts_audio`、`assemble_final` 与 `audit` 均为 `pending`；provider boundary 显示仅 1 次 `/scenario/s4` submit，poyo Seedance video job=1、poyo image generation=0、provider retry=0、禁止项总数为 `0`。文件系统 readback 显示 1 个 mp4 clip 位于 `/app/output/tenants/momcozy-marketing/pending_review/p1_s4_bounded_20260619T103337Z/`，`image_count=0`，`final_work_match_count=0`。原始二级 log gate 因把 completion summary 字段 `thumbnails=0` 误判为 thumbnail 执行而生成 `failed_or_blocked`；已基于同一份 backend log、API status 与生产 state/filesystem readback 纠偏为 `passed_corrected`，未执行第二次 submit。证据文件：`tmp/debug/todo-p1-3-corrected-live-summary-20260619T103337Z.json`、`tmp/debug/todo-p1-3-final-summary-20260619T103337Z.json`、`tmp/debug/todo-p1-3-live-backend-20260619T103337Z.log`、`tmp/debug/todo-p1-3-live-playwright-20260619T103337Z.log`、`tmp/debug/todo-p1-3-status-20260619T103337Z.json`、`tmp/debug/todo-p1-3-filesystem-20260619T103337Z.json`。证据等级为 `L4-authorized-live-provider-smoke`；本轮只证明 S4 bounded media 到 `seedance_clips`，不代表 S4 full media/final assembly、S1/S2/S3/S5 media、poyo image generation、TTS、thumbnail、assemble、media quality audit、gate、publish、delivery acceptance 或 approved brand token write 已执行。

2026-06-19 P1 S5 bounded media provider smoke 收口：`TODO-P1-4-prep` 已通过 PR `#39` 合并 S5 bounded media contract/readiness/live spec，并已按 `TODO-P1-4-sync-prep` 授权只同步 `src/routers/_state.py`、`src/routers/scenario.py` 与 `src/pipeline/s5_brand_vlog_pipeline.py` 到生产，只重启 `ai_video_backend`；生产 `/api/health=ok`，三文件本地/生产主机/容器 hash 一致，容器 introspection 通过，6 分钟 no-submit 日志窗口 forbidden total 为 `0`。随后用户授权执行 `TODO-P1-4` S5 bounded media single-submit provider smoke；本轮只运行 `web/e2e/production/scenario-s5-bounded-media-pilot-live.prod.spec.ts`，Playwright `1 passed (4.0m)`，`retries=0`、`workers=1`、临时 production key 已撤销，post-revoke protected check 返回 `401`，本地明文 key 文件已删除。生产 readback 显示 `pipeline_degraded=false`，`vlog_strategy`、`continuity_storyboard_grid`、`video_prompts` 与 `seedance_clips` 均为 `done`，`tts_audio`、`assemble_final` 与 `audit` 均为 `pending`；provider boundary 显示仅 1 次 `/scenario/s5` submit，poyo image job=0、poyo Seedance video job=1、provider retry=0、禁止项总数为 `0`。文件系统 readback 显示 1 个 mp4 clip 位于 tenant-scoped `pending_review`，`image_count=0`，`final_work_match_count=0`。证据文件：`tmp/debug/todo-p1-4-final-summary-20260619T132127Z.json`、`tmp/debug/todo-p1-4-live-backend-20260619T132127Z.log`、`tmp/debug/todo-p1-4-live-playwright-20260619T132127Z.log`、`tmp/debug/todo-p1-4-status-20260619T132127Z.json`、`tmp/debug/todo-p1-4-filesystem-20260619T132127Z.json`。证据等级为 `L4-authorized-live-provider-smoke`；本轮只证明 S5 bounded media 到 `seedance_clips`，不代表 S5 full media/final assembly、S1/S2/S3/S4 full media、TTS、thumbnail、assemble、media quality audit、gate、publish、delivery acceptance 或 approved brand token write 已执行。
...
|---|---|---|---|---|
| TODO-P1-1 | S1 bounded media single-submit smoke | completed_l4_authorized_live_bounded_passed | 已按授权完成 after-trace-id-sync 单 spec、单 submit live smoke；后续任何重跑或扩大到 full media 仍需重新授权预算、submit/job cap、artifact disposition 与止损 | Playwright `1 passed (6.0m)`；DeepSeek text calls=2，poyo image job=1，poyo Seedance job=1，provider retry=0；产物位于 tenant-scoped `pending_review`，`final_work=0`；`seedance_clips=done` 后停止，TTS/thumbnail/assemble/audit/gate/publish/delivery/token write 均未执行 |
| TODO-P1-2 | S3 bounded media single-submit smoke | completed_l4_authorized_live_bounded_passed_with_model_drift_followup | 已按授权完成 production sync-prep、no-submit verification、单 spec/单 submit live smoke 与 `P1-2Z` no-provider model 口径复核；后续任何重跑、扩大到 full media 或执行 generic/full-chain 视频 smoke 仍需重新授权预算、submit/job cap、artifact disposition 与止损 | Playwright `1 passed (4.4m)`；poyo image job=1，video job=1，provider retry=0；产物位于 tenant-scoped `pending_review`，clip verification `all_ok=true`；`seedance_clips=done` 后停止，TTS/thumbnail/assemble/audit/gate/publish/delivery/token write 均未执行；S3 使用 `ModelRouter` 显式 `kling-3.0/standard`，不受生产全局 `POYO_VIDEO_MODEL=happy-horse` 影响，但该 env 漂移需在 generic/full-chain 视频 smoke 前收口 |
...
| TODO-P1-6 | S1 gate / step-by-step 真实 token flow | completed_l4_authorized_live_s1_gate_candidates_passed | no-provider readiness 已完成；PR `#45` 修正 gate live spec 停点，PR `#53` 增加 timeout guard；DeepSeek 充值后 `P1-6A-R2S` 已按单标题、单链路授权通过 | `P1-6A-R2S` 只运行 `web/e2e/production/s1-gate.prod.spec.ts` 的 `step 2: gate exists after scripts and exposes 3 candidates @token-smoke`。Playwright `1 passed (1.3m)`；应用日志中 `/scenario/s1/start`、`/step/strategy`、`/step/scripts`、`gate_1_script/generate` 各 1 次；DeepSeek HTTP `200 OK` 计数为 `8`，provider abnormal count 为 `0`；poyo/Seedance/TTS/thumbnail/assemble/media_quality_audit/final_work/publish/delivery/approved brand token 均为 `0`。临时 key 已撤销，post-revoke auth check 为 `401`，本地明文 env 已删除。边界：只证明 S1 scripts 后 gate candidate generation 产出 3 candidates，不代表 gate approve/regenerate、media generation、完整 token suite、publish 或 delivery acceptance |

### P2：工程韧性与产品闭环

| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | platform_metrics_pull_blocked_by_no_active_post_source_and_shopify_post_dimension | 本地/fixture readiness、生产 authenticated dashboard readback、`/dashboard` 前端 read-only、`/metrics/pull` fail-closed 生产同步均已完成；`TODO-P2-1L1/L2/L3` 已完成本地 no-provider 的 platform fetcher / poller ingestion / webhook receiver contract；`TODO-P2-1L4` 已完成生产 read-only regression；`TODO-P2-1L5R` 已完成 1 次 authorized live webhook-only synthetic `audit.completed` receiver readback；`TODO-P2-1L6A` 已完成本地 connector contract / error classification / dry-run readiness 修复；`TODO-P2-1L6C` 已实现 TikTok / Shopify 真实 API mapping code path 并通过 mocked HTTP/no-provider 合同测试；`TODO-P2-1L6G/L6H` 已明确 active source 合同并同步到生产做 no-pull readiness，但生产 `video_metrics` 与 `publish_logs` 候选均为 `0`，ShopifyQL post-level 过滤维度未确认；真实 metrics pull、startup scheduler 与平台 metrics 数据流仍未执行 | `TODO-P2-1L1/L2` 本地目标测试 `6 passed`；`TODO-P2-1L3` webhook contract `18 passed`；metrics / webhook / analytics no-provider 回归集 `118 passed`；`TODO-P2-1L4` 生产 GET `/api/health`、`/api/dashboard/overview?days=7`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent` 均为 `200`；`TODO-P2-1L5R` receiver readback 新增 `1` 次 POST；`TODO-P2-1L6A` 本地目标测试 `19 passed`；`TODO-P2-1L6C` 目标测试 `29 passed`；`TODO-P2-1L6G` 目标测试 `31 passed`；`TODO-P2-1L6H` 生产 hash/import/no-pull readiness 通过且 forbidden log gate 为 `0`；不得外推为真实 TikTok/Shopify metrics pull、生产 scheduler、publish、delivery acceptance 或 approved brand token write |
| TODO-P2-2 | 多租户并发与 API key 隔离压测 | production_readonly_isolation_passed_with_default_global_compatibility | 本地 unit/fixture 层已通过；生产只读 key/state/portfolio 探针已通过；仍不运行 locust 生产压测 | 本地 `24 passed`；生产 `TODO-P2-2B` 创建并撤销 2 个临时 key，health/dashboard GET 均为 `200`，cross-tenant S2 state 读均 fail-closed 为 `404`，普通 tenant portfolio 仅见自身 pending_review，`default` key 的 global 可见性按兼容边界记录；backend forbidden log gate 为 `0`；无 provider、submit、output write、publish/delivery/token write |
| TODO-P2-3 | Quality ML 依赖生产可用性验证 | production_container_no_provider_verified | 本地 import smoke 与目标 pytest 集已通过；生产容器 one-off smoke 已通过；未下载权重、未 provider、未 submit | 本地目标集 `54 passed`；生产 `/api/health media_tools` bool contract 通过；容器 import smoke 全部成功；CLIP fallback contract 通过且未触发权重下载；backend forbidden log gate 为 `0`；证据等级为 `L3-production-container-read-only-no-provider`，不外推真实 ML 权重推理质量 |
...
- `L4D-5Y` 只证明 S2 bounded media 到 `seedance_clips`；`P1-5C-1R` 只证明 S2 segmented TTS 到 `tts_audio`；`P1-5C-2` 只证明 S2 segmented thumbnail 到 `thumbnail_images`。三者均不证明 S2 assemble、media quality audit 或 full media/final assembly。
- `L4D-5Z` 只证明 portfolio/library read-only 可见性与 `final_work=0`，不证明发布可用。
- `TODO-P1-1`、`TODO-P1-2`、`TODO-P1-3` 与 `TODO-P1-4` 只证明 S1/S3/S4/S5 bounded media 到 `seedance_clips`，不证明任一场景的 full media/final assembly、TTS、thumbnail、assemble、quality audit、publish 或 delivery acceptance。
...
与 `/api/metrics/{video_id}` 均返回 200，post-revoke 返回 401。`TODO-P2-1F` 进一步发现并
本地修复 dashboard 响应契约漂移：后端保留 `data` 字段，同时新增前端
`PerformanceDashboard` 需要的 `videos/scenarios/platforms`。`TODO-P2-1G` 已将该合同修复同步到
生产，authenticated GET 与浏览器页面上下文 readback 均验证 `/api/dashboard/overview?days=7`
...
真实 platform 数据流未验证。`TODO-P2-1L` 已新增
[`metrics-webhook-analytics-event-chain-plan-stable.md`](../workflows/metrics-webhook-analytics-event-chain-plan-stable.md)，
将下一步拆成 platform fetcher contract、poller ingestion contract、webhook receiver contract、
生产 read-only regression、外部 webhook single-event smoke、platform metrics pull single-post pilot
和 scheduler readiness；当前 `L1/L2/L3` 本地合同、`L4` 生产 read-only regression 和 `L5R` external webhook single-event smoke 已完成。`P2-1L6-prep` no-pull probe 判定 `L6` 当时不 ready：TikTok / Shopify fetcher 仍是 stub，生产 active post 候选为 `0`，且 Shopify 生产 env 名称与代码读取名存在漂移。`P2-1L6A` 已完成本地 connector contract / error classification / dry-run readiness 修复；`P2-1L6C` 已继续补上 TikTok / Shopify 真实 API mapping code path 并通过 mocked HTTP/no-provider 合同测试。默认下一步仍不是 live pull，阻塞项为生产 active post 候选仍为 `0`，且 ShopifyQL post-level 过滤维度尚未确认；否则继续保持 `METRICS_PULL_ENABLED=false`，不调用 `/api/metrics/pull`。
...
与 failure isolation；测试使用 mock 网络依赖。`P2-1L4` 只证明生产只读 dashboard / metrics readback 与日志禁止项，不涉及外部 receiver。`P2-1L5` 首次尝试创建了临时 webhook.site receiver，但 one-off 在发出前因生产 `WebhookManager` 构造函数参数不兼容失败，receiver 收到 `0` 次 POST；`P2-1L5R` 已用生产兼容 one-off 完成 1 次 synthetic `audit.completed` 外部 receiver readback，receiver 新增 `1` 次 POST，envelope 与 payload 精确匹配。Phase D 期间生产 `WEBHOOK_URLS` 仍为空，本次 webhook 注册仅存在于 one-off 进程内；仍未执行真实 publish / pipeline.completed 业务事件。
