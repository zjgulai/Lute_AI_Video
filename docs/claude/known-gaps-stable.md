...
| TODO-P1-5 | S2 full-media 分段计划 | p1_5d_prep_refs_only_assemble_readiness_passed_next_sync_or_live_auth | `media_stop_step` 合同与 no-provider 越界测试已完成；`P1-5C-1R` 已完成 S2 segmented `tts_audio` 单 submit live smoke；`P1-5C-2` 已完成 S2 segmented `thumbnail_images` 单 submit live smoke；`TODO-P1-5D-prep` 已完成 assemble refs-only 合同与 readiness guard，真实 assemble live 仍未执行 | TTS segment：DeepSeek calls=2、TTS job=1、provider retry=0、产物进入 tenant-scoped `pending_review`、`final_work=0`；thumbnail segment：poyo thumbnail image job=1、Seedance/TTS/keyframe/assemble/audit/gate/publish/delivery/token write 均未执行、`final_work=0`；assemble prep：`provider_job_caps={}`、refs-only、no submit/no provider；后续 P1-5D live / P1-5E audit 仍需单独授权、预算、止损 |
