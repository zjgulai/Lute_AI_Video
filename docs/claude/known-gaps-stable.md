...
| TODO-P1-5 | S2 full-media 分段计划 | p1_5c_1r_tts_segment_l4_passed_next_thumbnail_blocked | `media_stop_step` 合同与 no-provider 越界测试已完成；`P1-5C-1R` 已按授权完成 S2 segmented `tts_audio` 单 submit live smoke；下一步只能逐段授权 thumbnail / assemble / audit live smoke | TTS segment：DeepSeek calls=2、TTS job=1、provider retry=0、产物进入 tenant-scoped `pending_review`、`final_work=0`；poyo image/Seedance/thumbnail/assemble/audit/gate/publish/delivery/token write 均未执行；后续每个 stop point 仍需单独 spec、单独预算、单独止损 |
...
- `L4D-5Y` 只证明 S2 bounded media 到 `seedance_clips`；`P1-5C-1R` 只证明 S2 segmented TTS 到 `tts_audio`。二者均不证明 S2 thumbnail、assemble、media quality audit 或 full media/final assembly。
