...
2026-06-17 P2 Quality ML 本地 no-provider readiness：`TODO-P2-3A` 已执行但不能标记为通过。使用 `DATABASE_URL=`、`HF_HUB_OFFLINE=1`、`TRANSFORMERS_OFFLINE=1` 运行本地 import smoke，`src.quality.clip_alignment`、`src.quality.nr_quality`、`src.quality.scene_analysis`、`src.quality.face_consistency`、`src.quality.safe_zone`、`src.skills.media_quality_audit`、`src.routers.health` 均 import 成功，`/health` 的 `media_tools` contract 可返回，`ClipAligner` 初始化仍保持 lazy-load，不下载权重；canonical sanitized summary 为 `tmp/debug/todo-p2-3a-quality-ml-local-readiness-20260617082850.json`，closeout summary 为 `tmp/debug/todo-p2-3a-quality-ml-local-closeout-20260617083033.json`。随后按授权运行 `DATABASE_URL= HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 .venv/bin/pytest tests/test_health_media_tools.py tests/test_clip_alignment.py tests/test_media_tools.py tests/test_quality_signals.py -q`，结果为 `51 passed, 2 failed`。失败项均在 `tests/test_media_tools.py`：`TestVideoDownloaderMockMode.test_transcribe_returns_mock_segments` 和 `TestVideoDownloaderMockMode.test_download_and_transcribe_returns_both`。根因是当前本地 `.venv` 已安装 `faster-whisper`，`VideoDownloader._check_whisper()` 返回可用后，mock-mode 测试传入的假路径进入真实 `faster_whisper`/PyAV 解码并触发 `FileNotFoundError`；这暴露的是 mock-mode 测试隔离脆弱点，不是 provider 调用。证据等级保持 `L2-fixture-or-dry-run`；本轮未访问生产容器、未调用 provider、未执行 `/api/scenario/*` submit 或 Fast Mode submit。
...
| TODO-P2-3 | Quality ML 依赖生产可用性验证 | local_import_passed_pytest_failed | 本地 import smoke 已通过；授权 pytest 集因 `tests/test_media_tools.py` mock-mode 与已安装 `faster-whisper` 冲突失败；生产容器 smoke 未执行 | canonical summary `tmp/debug/todo-p2-3a-quality-ml-local-readiness-20260617082850.json`；pytest `51 passed, 2 failed`，失败为 fake video path 进入真实 faster-whisper/PyAV 解码；下一步先修复 mock-mode 测试隔离或显式 mock whisper，再复跑本地 no-provider readiness |
