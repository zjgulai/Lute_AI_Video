...
| TODO-P1-5 | S2 full-media 分段计划 | p1_5c_2_thumbnail_segment_l4_passed_next_assemble_blocked | `media_stop_step` 合同与 no-provider 越界测试已完成；`P1-5C-1R` 已完成 S2 segmented `tts_audio` 单 submit live smoke；`P1-5C-2` 已完成 S2 segmented `thumbnail_images` 单 submit live smoke；下一步只能逐段授权 assemble / audit live smoke | TTS segment：DeepSeek calls=2、TTS job=1、provider retry=0、产物进入 tenant-scoped `pending_review`、`final_work=0`；thumbnail segment：poyo thumbnail image job=1、Seedance/TTS/keyframe/assemble/audit/gate/publish/delivery/token write 均未执行、`final_work=0`；后续每个 stop point 仍需单独 spec、单独预算、单独止损 |
