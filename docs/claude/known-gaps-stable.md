...
| ID | 任务 | 当前状态 | 执行边界 | 验收口径 |
|---|---|---|---|---|
| TODO-P2-1 | metrics / webhook / analytics 真实闭环 | external_webhook_attempt_failed_before_send | 本地/fixture readiness、生产 authenticated dashboard readback、`/dashboard` 前端 read-only、`/metrics/pull` fail-closed 生产同步均已完成；`TODO-P2-1L1/L2/L3` 已完成本地 no-provider 的 platform fetcher / poller ingestion / webhook receiver contract；`TODO-P2-1L4` 已完成生产 read-only regression；`TODO-P2-1L5` 首次尝试失败于 webhook 发出前，真实 metrics pull、外部 webhook dispatch、startup scheduler 与平台 metrics 数据流仍未执行 | `TODO-P2-1L1/L2` 本地目标测试 `6 passed`；`TODO-P2-1L3` webhook contract `18 passed`；metrics / webhook / analytics no-provider 回归集 `118 passed`；`TODO-P2-1L4` 生产 GET `/api/health`、`/api/dashboard/overview?days=7`、`/api/metrics/todo-p2-1-readonly-probe-nonexistent` 均为 `200`，post-revoke 为 `401`，backend log gate 禁止项均为 `0`；`TODO-P2-1L5` receiver readback 为 `0` 次 POST，失败根因为生产 `WebhookManager` 构造函数不支持 `timeout_seconds`；下一步只能另行授权 `P2-1L5R` retry，不得外推为真实 TikTok/Shopify metrics pull、webhook.site 成功回读、生产 scheduler、publish、delivery acceptance 或 approved brand token write |
...
和 scheduler readiness；当前 `L1/L2/L3` 本地合同和 `L4` 生产 read-only regression 已完成，`L5` 首次尝试失败于发送前且 receiver 收到 `0` 次 POST，默认下一步是另行授权的 `L5R` external webhook retry，不启用生产 metrics pull。
...
与 failure isolation；测试使用 mock 网络依赖。`P2-1L4` 只证明生产只读 dashboard / metrics readback 与日志禁止项，不涉及外部 receiver。`P2-1L5` 首次尝试创建了临时 webhook.site receiver，但 one-off 在发出前因生产 `WebhookManager` 构造函数参数不兼容失败，receiver 收到 `0` 次 POST。Phase D 期间生产 `WEBHOOK_URLS` 为空，仍从未成功触发真实外部接收端。需要另行授权 `L5R`，使用生产兼容 one-off 或先同步 webhook manager 后再跑受控事件验证。
