...
- `2026-06-13`: L4D-5W-R2 frontend/library endpoint-clean read-only regression passed after separate explicit authorization. Evidence: `tmp/debug/l4d5wr2-live-playwright-20260613050712.log`, `tmp/debug/l4d5wr2-live-summary-20260613050712.json`, and `tmp/debug/l4d5wr2-refined-log-gate-20260613050712.json`. Result: only `web/e2e/production/library-portfolio.prod.spec.ts` ran, Playwright `3 passed`, `RUN_TOKEN_SMOKE=0`, `--retries=0`, and `--workers=1`; backend log showed only `/portfolio/` GETs. The temporary non-demo production key for tenant `momcozy-marketing` was revoked and post-revoke authentication returned `401`. The formal log gate is now `scripts/production_readonly_log_gate.py`; it allows local health noise (`127.0.0.1 /health` and current renderer UDS health, while retaining `rendering:3001/health` solely for historical log replay) but continues to fail external health/admin/media requests, scenario/Fast submit, provider, publish, delivery, and approved brand token logs.
