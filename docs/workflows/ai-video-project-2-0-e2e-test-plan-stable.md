...
| `Video 2.0 production no-provider deploy baseline` | L4 deployment + L3 read-only regression | 否 | 将 main 同步并部署到 Lighthouse，验证 2.0.0、health、smoke、strict read-only E2E、核心页面和日志边界 | 2026-06-25 已通过；PR `#55` merge commit `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9`，生产 `/api/health.version=2.0.0`，main checks 全绿，strict read-only E2E `55 passed`，provider/submit/publish/delivery/token write/final_work 写入计数为 0 |
...
[事实] 2026-06-25 已完成 Video 2.0 production no-provider deployment baseline。PR `#55` 已合并，main SHA 为 `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9`；生产 `/api/health` 返回 `status=ok`、`version=2.0.0`、PostgreSQL healthy、Remotion available；GitHub main checks `CI`、`e2e-ui`、`Deploy to GitHub Pages`、`e2e-prod` 均为 success；`deploy/lighthouse/smoke.sh` 在 `RUN_TOKEN_SMOKE=0` 下通过；strict read-only production E2E 为 `55 passed`；`/`、`/toolbox`、`/dashboard`、`/library`、`/works`、`/settings` 均返回 `200`。log gate 显示 provider HTTP、`/api/scenario/*` submit、Fast Mode submit、publish/delivery、approved brand token write、`final_work` 写入均为 `0`。证据文件为 `tmp/debug/video20-production-deploy-summary-20260625T013842Z.json`、`tmp/debug/video20-version-deploy-summary-20260625T021617Z.json` 与 `tmp/debug/video20-version-deploy-backend-log-20260625T0208Z.log`。本轮仍不能声明 provider full chain、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已完成。
