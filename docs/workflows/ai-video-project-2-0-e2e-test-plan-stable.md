...
1. 为 `/settings` 增加 Playwright 页面级 smoke，覆盖后端离线、provider 分类、key masked 展示。
2. 为 `api.ts` 增加请求 payload 审计测试，确保 disabled provider 和空 key 不会进入后端。
3. 为生产非 token E2E 增加 `/settings` 路由覆盖，不改变 `@token-smoke` 默认跳过策略。
