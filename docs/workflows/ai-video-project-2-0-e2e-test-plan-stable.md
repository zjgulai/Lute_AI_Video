...
- L3 生产非 token E2E 使用非 demo production key 通过，且结果不低于当前可执行基线（已在无 key 场景更新为 `39 passed, 15 skipped`）；`library-portfolio` 断言需在非 demo key 下实跑命中。
- `/portfolio` 与 `Materials` tab 的 pending_review 只读边界已覆盖：`creation_intermediate` 与 `pending_review` 可见，`/library?tab=materials` 未发起 `POST/PUT/PATCH/DELETE`。
