...
| `Video 2.0 production no-provider deploy baseline` | L4 deployment + L3 read-only regression | 否 | 将 main 同步并部署到 Lighthouse，验证 2.0.0、health、smoke、strict read-only E2E、核心页面和日志边界 | 2026-06-25 已通过；PR `#55` merge commit `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 完成生产部署/版本同步，生产 `/api/health.version=2.0.0`；随后 docs-only `38276014eb87a0a0bb37e8202a0cc07869fd980d` 推送后 `HEAD` / `origin/main` / GitHub main 对齐。`bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 上 `CI` / `e2e-ui` / `Deploy to GitHub Pages` / `e2e-prod` 全绿，`38276014eb87a0a0bb37e8202a0cc07869fd980d` 上 `CI` / `Deploy to GitHub Pages` 全绿；strict read-only E2E `55 passed`，30 分钟 post-deploy monitor 仍 clean，provider/submit/publish/delivery/token write/final_work 写入计数为 0 |
...
[事实] 2026-06-25 已完成 Video 2.0 production no-provider deployment baseline。PR `#55` 已合并，生产部署/版本同步 SHA 为 `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9`；随后 docs-only 收口提交 `38276014eb87a0a0bb37e8202a0cc07869fd980d` 已推送，当前 `HEAD` / `origin/main` / GitHub main 对齐到 `38276014eb87a0a0bb37e8202a0cc07869fd980d`。生产 `/api/health` 返回 `status=ok`、`version=2.0.0`、PostgreSQL healthy、Remotion available；`bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 上 GitHub checks `CI`、`e2e-ui`、`Deploy to GitHub Pages`、`e2e-prod` 均为 success；`38276014eb87a0a0bb37e8202a0cc07869fd980d` 上 `CI` 与 `Deploy to GitHub Pages` 为 success。`deploy/lighthouse/smoke.sh` 在 `RUN_TOKEN_SMOKE=0` 下通过；strict read-only production E2E 为 `55 passed`；`/`、`/toolbox`、`/dashboard`、`/library`、`/works`、`/settings` 均返回 `200`。log gate 显示 provider HTTP、`/api/scenario/*` submit、Fast Mode submit、publish/delivery、approved brand token write、`final_work` 写入均为 `0`；后续 30 分钟 post-deploy monitor 继续显示 `traceback_or_critical_or_5xx_or_429_count=0` 且所有禁止项为 `0`。证据文件为 `tmp/debug/video20-production-deploy-summary-20260625T013842Z.json`、`tmp/debug/video20-version-deploy-summary-20260625T021617Z.json`、`tmp/debug/video20-version-deploy-backend-log-20260625T0208Z.log` 与 `tmp/debug/video20-post-deploy-monitor-summary-20260625T023331Z.json`。本轮仍不能声明 provider full chain、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已完成。
...
| `P1-5C-2` thumbnail segment smoke | `thumbnail_prompts` + `thumbnail_images` 单缩略图执行 | 已完成；thumbnail image job = 1 | 已禁止并验证 Seedance、TTS、keyframe image、assemble、audit、final_work |
| `P1-5D` assemble segment smoke | 用已存在 pending_review clip/audio/thumb refs 组装中间产物 | 已完成：refs-only local Remotion assemble = 1，provider HTTP = 0 | provider、publish、delivery、approved brand token |
| `P1-5E` audit segment smoke | 对已存在中间产物做 media_quality_audit | 已执行 1 次 refs-only live side effect；provider HTTP=0；旧 spec 期望漂移已由 PR `#44` 对齐 | provider、publish、delivery、approved brand token；修正后 live green 证据仍需重新授权 |
...
[事实] `P1-5C-2` 已按用户授权完成 S2 segmented `thumbnail_images` 单 submit live segment。只运行 `web/e2e/production/scenario-s2-thumbnail-segment-live.prod.spec.ts`，`RUN_TOKEN_SMOKE=1`、`PLAYWRIGHT_MAX_SUBMIT_COUNT=1`、`PLAYWRIGHT_PROVIDER_MAX_RETRIES=0`、`PLAYWRIGHT_ARTIFACT_DISPOSITION=pending_review`、`PLAYWRIGHT_S2_SEGMENTED_MEDIA_STOP_STEP=thumbnail_images`。本轮只执行 1 次 `/api/scenario/s2` submit，允许 DeepSeek 文本与 1 次 poyo `gpt-image-2` thumbnail image generation；禁止 keyframe image、Seedance、TTS、assemble、media_quality_audit、gate、publish、delivery 与 approved brand token。Playwright return code 为 `0`，状态 readback 显示 `strategy`、`scripts`、`thumbnail_prompts`、`thumbnail_images` 为 `done`，`keyframe_images`、`seedance_clips`、`tts_audio`、`assemble_final` 与 `audit` 均未执行。文件系统 readback 显示 1 张 thumbnail 位于 tenant-scoped `pending_review`，`video_file_count=0`、`audio_file_count=0`、`matching_final_work_count=0`。原始一次性收口脚本因重复计数同一次 POST 与 revoke helper import 漂移而判失败；corrected closeout 已按 `key_id` 撤销临时 production key，`revoked=true`，本地明文 key 文件和 raw log 已删除。

[证据] `P1-5C-2` evidence：

- `tmp/debug/todo-p1-5c-thumbnail-closeout-20260621T051833Z.json`：`provider_smoke_decision=passed`，refined log gate 显示 `refined_s2_submit_count=1`、`poyo_submit_http_count=1`、`forbidden_hits={}`。
- `tmp/debug/todo-p1-5c-thumbnail-status-20260621T051833Z.json`：status readback `200`，`thumbnail_images=done`，downstream/forbidden steps 未执行。
- `tmp/debug/todo-p1-5c-thumbnail-filesystem-20260621T051833Z.json`：tenant-scoped `pending_review` thumbnail 文件存在，`image_file_count=1`，`video_file_count=0`，`audio_file_count=0`，`matching_final_work_count=0`。
- `tmp/debug/todo-p1-5c-thumbnail-backend-20260621T051833Z.log`：DeepSeek 文本请求与 1 次 poyo `gpt-image-2` submit；无 keyframe image、Seedance、TTS、assemble、media_quality_audit、final_work、publish、delivery。
- `tmp/debug/todo-p1-5c-thumbnail-final-summary-20260621T051833Z.json`：保留原始 automation 判失败原因，便于后续修正 live-smoke harness。

[边界] `TODO-P1-5A` 证明本地 S2 segmented stop-point 合同、step 越界防护和 readiness guard 通过；`P1-5C-1R` 只证明 S2 segmented TTS 到 `tts_audio` 的 `L4-authorized-live-provider-tts-only`；`P1-5C-2` 只证明 S2 segmented thumbnail 到 `thumbnail_images` 的 `L4-authorized-live-provider-thumbnail-only`。仍不代表 S2 assemble、media_quality_audit、full media、final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

[事实] `TODO-P1-5D-prep` 已按 no-provider 范围完成 S2 segmented `assemble_final` refs-only 合同与 readiness。为避免 assemble segment 为了补齐上游状态而触发 DeepSeek、poyo image、Seedance、TTS 或 thumbnail，`S2BrandCampaignRequest` 新增 `media_refs`，router 显式传入 `body.media_refs`，S2 pipeline 在 `media_stop_step=assemble_final` 时要求 refs 指向 tenant-scoped `pending_review` 或 `quarantine`，拒绝 `final_work`、`renders`、`fast_mode` 与 `gpt_images` 路径；step order 收窄为只执行 `assemble_final`，`provider_job_caps={}`，并将 assemble 输出搬移到 `tenants/{tenant}/{disposition}/{label}/assemble/`。对外结果只暴露 `intermediate_video_path` / `intermediate_render_json_path`，`final_video_path` 与 `render_json_path` 仍为空，`delivery_accepted=false`、`publish_allowed=false`、`approved_brand_token_write=false`。

[证据] `TODO-P1-5D-prep` evidence：

- `DATABASE_URL= HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 .venv/bin/pytest tests/test_s2_e2e.py -q`：`37 passed`。
- `.venv/bin/ruff check src/routers/_state.py src/routers/scenario.py src/pipeline/s2_brand_pipeline_v2.py tests/test_s2_e2e.py`：通过。
- `cd web && RUN_TOKEN_SMOKE=0 npx playwright test -c playwright.prod.config.ts e2e/production/scenario-s2-assemble-segment-readiness.prod.spec.ts --list`：只枚举 1 个 readiness 测试。
- `cd web && RUN_TOKEN_SMOKE=0 npx playwright test -c playwright.prod.config.ts e2e/production/scenario-s2-assemble-segment-readiness.prod.spec.ts --reporter=list`：`1 passed`。
- `cd web && RUN_TOKEN_SMOKE=1 npx playwright test -c playwright.prod.config.ts e2e/production/scenario-s2-assemble-segment-live.prod.spec.ts --list`：只枚举 1 个 `@token-smoke` live 测试；本轮未执行 live submit。
- `cd web && npx vitest run src/lib/prodE2eTokenGuard.test.ts`：`7 passed`。

[边界] `TODO-P1-5D-prep` 只证明本地 refs-only assemble 合同、readiness guard 与 token-smoke 默认隔离；不代表生产已同步，不代表真实 Remotion/local assemble live 已执行，不代表 S2 media_quality_audit、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

[事实] `P1-5D` 已按用户授权完成 S2 segmented `assemble_final` refs-only single-submit smoke。只运行 `web/e2e/production/scenario-s2-assemble-segment-live.prod.spec.ts`，`RUN_TOKEN_SMOKE=1`、`PLAYWRIGHT_MAX_SUBMIT_COUNT=1`、`PLAYWRIGHT_PROVIDER_MAX_RETRIES=0`、`PLAYWRIGHT_ARTIFACT_DISPOSITION=pending_review`、`PLAYWRIGHT_S2_SEGMENTED_MEDIA_STOP_STEP=assemble_final`，并显式传入既有 pending_review clip/audio/thumbnail refs。run label 为 `p1_s2_segmented_assemble_20260621T153042Z`，Playwright `1 passed`，仅 1 次 `/api/scenario/s2` submit；provider HTTP、DeepSeek、poyo、Seedance provider、TTS、thumbnail generation、keyframe generation、media_quality_audit 与 gate candidate 计数均为 `0`；只执行允许的 local Remotion assemble `1` 次。文件系统 readback 显示 `pending_review` 下生成 1 个 mp4 和 1 个 render input JSON，`matching_final_work_count=0`；临时 production key 已撤销，post-revoke protected check 返回 `401`，本地明文 key 文件已删除。

[证据] `P1-5D` evidence：

- `tmp/debug/p1-5d-assemble-live-adjudication-20260621T153042Z.json`：corrected adjudication 为 `passed`，`scenario_submit_count=1`，`log_gate_passed=true`，forbidden provider/action 计数均为 `0`。
- `tmp/debug/p1-5d-assemble-playwright-20260621T153042Z.log`：目标 spec `1 passed`。
- `tmp/debug/p1-5d-assemble-backend-20260621T153042Z.log`：单次 `/scenario/s2` submit，`POST http://rendering:3001/assemble` 为 `200`，无外部 provider HTTP。
- `tmp/debug/p1-5d-assemble-status-20260621T153042Z.json`：status readback `200`，`scenario=s2`，`status=completed`，`pipeline_degraded=false`，`steps.assemble_final.output.refs_only=true`。
- `tmp/debug/p1-5d-assemble-filesystem-20260621T153042Z.json`：tenant-scoped `pending_review` 文件存在，`video_file_count=1`，`render_json_count=1`，`audio_file_count=0`，`image_file_count=0`，`matching_final_work_count=0`。

[边界] `P1-5D` 只证明 S2 segmented refs-only local assemble 到 `assemble_final` 的 `L4-authorized-live-local-assemble-only`；不代表 S2 media_quality_audit、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

[事实] `TODO-P1-5E-prep` 已按 no-provider 范围完成 S2 segmented `audit` refs-only/read-only 合同与 readiness。为避免 audit segment 为补齐上游状态而触发 DeepSeek、poyo image、Seedance、TTS、thumbnail 或 assemble，`S2_SEGMENTED_MEDIA_STEP_ORDERS["audit"]` 已收窄为只执行 `audit`，`provider_job_caps={}`。`media_stop_step=audit` 必须显式提供 `media_refs`，refs 只能指向 tenant-scoped `pending_review` 或 `quarantine`，并要求 clip、audio、thumbnail 与 assemble video；可选 render JSON、lyrics、thumbnail prompts、scripts、storyboards 与 continuity grid。运行前通过 `_seed_refs_only_audit_inputs` 将既有 refs 注入 state，state/config 标记 `refs_only_media_audit=true`；结果仍保持 `final_video_path=""`、`render_json_path=""`、`delivery_accepted=false`、`publish_allowed=false`、`approved_brand_token_write=false`。新增 readiness/live spec：`web/e2e/production/scenario-s2-audit-segment-readiness.prod.spec.ts` 与 `web/e2e/production/scenario-s2-audit-segment-live.prod.spec.ts`。

[证据] `TODO-P1-5E-prep` evidence：

- `DATABASE_URL= HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 .venv/bin/pytest tests/test_s2_e2e.py -q`：`39 passed`。
- `.venv/bin/ruff check src/pipeline/s2_brand_pipeline_v2.py tests/test_s2_e2e.py`：通过。
- `cd web && RUN_TOKEN_SMOKE=0 npx playwright test -c playwright.prod.config.ts e2e/production/scenario-s2-audit-segment-readiness.prod.spec.ts --reporter=list`：`1 passed`。
- `cd web && RUN_TOKEN_SMOKE=1 npx playwright test -c playwright.prod.config.ts e2e/production/scenario-s2-audit-segment-live.prod.spec.ts --list`：只枚举 1 个 `@token-smoke` live 测试；本轮未执行 live submit。
- `cd web && npx vitest run src/lib/prodE2eTokenGuard.test.ts`：`7 passed`。
- `git diff --check`：通过。

[边界] `TODO-P1-5E-prep` 只证明本地 refs-only audit 合同、readiness guard 与 token-smoke 默认隔离；不代表生产已同步，不代表真实 media_quality_audit live 已执行，不代表 S2 full media/final assembly、publish、delivery acceptance 或 approved brand token write 已执行。

[事实] `P1-5E-sync-prep` 已按授权完成生产同步与 no-submit verification。同步文件为 `src/routers/_state.py`、`src/routers/scenario.py`、`src/pipeline/s2_brand_pipeline_v2.py`；生产 `/api/health` 返回 ok，本地/远端/容器 hash verify 通过，容器 import/introspection 证明 S2 request model 包含 `artifact_disposition`、`provider_max_retries`、`output_label`、`media_stop_step`、`media_refs`，且 `audit` stop point 的 step order 为 `["audit"]`、`provider_job_caps={}`、`refs_only_media_audit_supported=true`。6 分钟 no-submit 窗口内 forbidden backend log pattern 为 `0`。证据为 `tmp/debug/p1-5e-sync-prep-summary-20260622T071200Z.json`。

[事实] `P1-5E` 已按用户授权执行 S2 segmented `audit` refs-only single-submit smoke。run label 为 `p1_s2_segmented_audit_20260622T074557Z`，只运行 `web/e2e/production/scenario-s2-audit-segment-live.prod.spec.ts`，`media_stop_step=audit`，显式传入既有 pending_review clip/audio/thumbnail/video/render JSON refs。生产状态 readback 显示 pipeline `status=completed`、`pipeline_degraded=false`、`audit=done`、`audit_report_present=true`、`audit_overall_status=FAIL`；该 FAIL 是 media quality audit 对短 refs/素材质量的业务评分结果，不是 provider 或 pipeline 执行失败。`continuity_storyboard_grid` 被 refs-only input 注入为 `done` 且 `output.status=refs_only`，`keyframe_images` 与 `video_prompts` 保持 `pending`。backend 日志显示 1 次实际 `POST /scenario/s2`；旧 summary 中 `scenario_submit_count=2` 是 middleware 与 uvicorn 对同一次 POST 的重复日志行计数。provider HTTP、DeepSeek、poyo、Seedance、TTS、thumbnail generation、keyframe generation、gate、final_work、publish/delivery 计数均为 `0`；临时 production key 已撤销，post-revoke protected check 返回 `401`。

[证据] `P1-5E` evidence：

- `tmp/debug/p1-5e-live-summary-20260622T074557Z.json`：记录 `decision=failed_contract_drift`，原因是旧 Playwright spec 期望 `continuity_storyboard_grid` 为 pending，而生产状态返回 refs-only done；同时记录 provider / publish / delivery 禁止项计数为 `0`。
- `tmp/debug/p1-5e-status-20260622T074557Z.json`：status readback `200`，`audit=done`，`audit_report_present=true`，`continuity_storyboard_grid.output.status=refs_only`，`keyframe_images` 与 `video_prompts` 为 pending。
- `tmp/debug/p1-5e-backend-log-20260622T074557Z.log`：1 次实际 `POST /scenario/s2` 与 status readback；无 provider HTTP / DeepSeek / poyo / Seedance / TTS / thumbnail / keyframe / assemble / gate / publish / delivery。
- `tmp/debug/p1-5e-revoke-key-20260622T074557Z.json` 与 `tmp/debug/p1-5e-post-revoke-auth-follow-redirect-20260622T074557Z.json`：临时 key 已撤销，post-revoke protected check 返回 `401`。
- PR `#44` merge commit `b30afc9`：将 Python/Playwright 合同对齐为接受 refs-only seeded `continuity_storyboard_grid`；PR checks 与 main checks 均为 success。

[边界] `P1-5E` 可声明生产 refs-only audit side effect 已执行、provider/publish/delivery 禁止边界通过、临时 key lifecycle 已闭环、测试合同漂移已通过 PR `#44` 修正。不得声明当次旧 Playwright spec 为 passed，也不得外推为修正后 live spec 已重新绿灯；如需要 green Playwright 证据，必须重新授权一次单 submit。
...
28. [x] 完成 `P1-5C-2` S2 segmented `thumbnail_images` live segment：单次 `/api/scenario/s2` submit，poyo thumbnail image job=1，provider retry=0，产物进入 tenant-scoped `pending_review`，`final_work=0`；keyframe image、Seedance、TTS、assemble、audit、publish、delivery、approved brand token 均未执行。
29. [x] 完成 `TODO-P1-5D-prep` S2 segmented `assemble_final` refs-only readiness：新增 `media_refs` 合同、refs-only assemble step order、`provider_job_caps={}`、review-scope 输出搬移与 readiness/live spec guard。
30. [x] 完成 `P1-5D` S2 segmented `assemble_final` refs-only live smoke：单次 `/api/scenario/s2` submit，provider HTTP / DeepSeek / poyo / Seedance / TTS / thumbnail / keyframe / audit / gate 均为 `0`，local Remotion assemble `1` 次，产物进入 tenant-scoped `pending_review`，`final_work=0`，临时 key 已撤销。
31. [x] 完成 `TODO-P1-5E-prep` S2 segmented `audit` refs-only readiness：新增 refs-only audit step order、`provider_job_caps={}`、review-scope refs 校验、`refs_only_media_audit` state/config 标记与 readiness/live spec guard。
32. [x] 完成 `P1-5E` S2 segmented `audit` refs-only live side-effect closeout：生产 sync-prep 通过，live run 只触发 1 次实际 `/scenario/s2` submit，audit readback 完成，provider/DeepSeek/poyo/Seedance/TTS/thumbnail/keyframe/assemble/gate/publish/delivery 均为 `0`；旧 spec 因 refs-only seeded step 状态口径漂移返回 nonzero，PR `#44` 已修正合同并合入 main。
