...
| `Video 2.0 production no-provider deploy baseline` | L4 deployment + L3 read-only regression | 否 | 将 main 同步并部署到 Lighthouse，验证 2.0.0、health、smoke、strict read-only E2E、核心页面和日志边界 | 2026-06-25 已通过；PR `#55` merge commit `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 完成生产部署/版本同步，生产 `/api/health.version=2.0.0`。后续 `ae094f45d9ea720d15194a4336a4a7ca86347186` 已完成 all-products Lighthouse 部署保护修复、部署源推送到 `origin/main`、生产只读复核与 `RUN_TOKEN_SMOKE=0` smoke。`bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 上 `CI` / `e2e-ui` / `Deploy to GitHub Pages` / `e2e-prod` 全绿；`ae094f45d9ea720d15194a4336a4a7ca86347186` 上 `CI` / `Deploy to GitHub Pages` 全绿。strict read-only E2E `55 passed`，provider/submit/publish/delivery/token write/final_work 写入计数为 0 |
...
[事实] 2026-06-25 已完成 Video 2.0 production no-provider deployment baseline。PR `#55` 已合并，生产部署/版本同步 SHA 为 `bad53cdd07ab80f580bceed06e3ee1d9fa7471a9`；随后 `ae094f45d9ea720d15194a4336a4a7ca86347186` 完成 Lighthouse `rsync --delete` remote-only sidecar 保护修复，并已推送到 `origin/main`。生产 `/api/health` 返回 `status=ok`、`version=2.0.0`、PostgreSQL healthy、Remotion available；`bad53cdd07ab80f580bceed06e3ee1d9fa7471a9` 上 GitHub checks `CI`、`e2e-ui`、`Deploy to GitHub Pages`、`e2e-prod` 均为 success。`deploy/lighthouse/smoke.sh` 在 `RUN_TOKEN_SMOKE=0` 下通过；strict read-only production E2E 为 `55 passed`；`/`、`/s1`、`/s2`、`/s3`、`/s4`、`/s5`、`/fast`、`/toolbox`、`/dashboard`、`/library`、`/works`、`/settings` 均返回 `200`。2026-06-26 00:11 复核显示 backend/frontend/rendering/nginx 均 running 且 restart=0；refined log gate 显示 provider HTTP、`/api/fast/generate`、`/api/scenario/*` submit 与 5xx 计数均为 `0`。证据文件包括 `tmp/debug/all-products-lighthouse-deploy-20260625T123349Z.log`、`tmp/debug/video20-production-deploy-summary-20260625T013842Z.json`、`tmp/debug/video20-version-deploy-summary-20260625T021617Z.json`、`tmp/debug/video20-version-deploy-backend-log-20260625T0208Z.log` 与 `tmp/debug/video20-post-deploy-monitor-summary-20260625T023331Z.json`。本轮仍不能声明 provider full chain、full media/final assembly、publish、delivery acceptance 或 approved brand token write 已完成。
