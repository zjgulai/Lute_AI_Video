...
AI Video 2.0 已形成一套可审计的商业视频工具链：商业合约、离线质量门禁、provider 信号、prompt compiler、生产任务账本、场景只读注入、prompt preview audit、品牌 token 审核、长视频审计、工具箱图像工具 L2 矩阵、工具级 provider readiness preflight、poyo 公开文档重验、authorized-live 预审/接线链路与受控 real smoke 证据均已对齐。当前状态从纯 `L2-fixture-or-dry-run` 收敛到“局部 `L4-authorized-live` 可复核状态”：`Momcozy 消毒器 3 图 + 1 视频` 已完成 provider 提交并写入待审资产，但仅限 `pending_review` 与样本范围边界。

当前最高证据等级为 `L4-authorized-live`（样本范围内、待审边界内）。该一次性授权 smoke 已满足：
- 范围：Momcozy 消毒器 3 张图片 + 1 条 15 秒 9:16 图片驱动视频；
- 预算：总计 `$3.00`，单 job `$2.50`，自动重试 0；
- 产物：4 个 job 均有 `artifact_ref/media_url/job_id`，`review_status=pending_review`；
- 禁止项：`delivery_accepted=false`、`publish_allowed=false`、`approved_brand_token_write=false`。

`C21-C35 + C36` 前置门禁仍然是下一次执行的真实执行门控；重复执行仍需以用户最新授权文本 + 私有 approval/account readiness record + 私有 payload + `CONFIRM_P2_TOKEN_SMOKE=1` + `RUN_TOKEN_SMOKE=1` + `AI_VIDEO_AUTHORIZED_LIVE_EXECUTE=1` + `AI_VIDEO_AUTHORIZED_LIVE_POYO_TRANSPORT=1` 组合为前置。
...
| C24-C30 poyo revalidation, sample plan, private records, smoke packet | `768cd1a`, `74c2d3b`, `52bc8cc`, `5eb655b`, `cbd2268`, `ecbae24`, `890d569` |
| C31-C36 runtime gating, submitter assembly, real submitter execution path | `f1b1bed`, `7d8fcb8`, `05c80c8`, `db5e47d`, `e1e075e`, `ff29727` |

Plan source: `docs/superpowers/plans/2026-06-04-ai-video-2-0-remaining-implementation-plan.md`.

## Evidence Grade

- **Current grade**: `L4-authorized-live`（作用域：Momcozy 消毒器资产包）。
- **Latest real execution evidence**: `tmp/outputs/authorized-live-poyo-smoke-rerun-20260607-summary.json` 显示 `evidence_level=L4-authorized-live`，`provider_call_executed=true`，`blocked_reasons=[]`，4 条 job 已提交并进入 `pending_review`，`delivery_accepted` / `publish_allowed` / `approved_brand_token_write` 均为 `false`。
- **Real call boundary**: 当前真实 provider 调用已经发生，但仍受 `pending_review`、非重试、固定样本、no publish 的运行约束控制，未进入品牌交付闭环。
...
- 已执行过一次受控 provider live smoke（momcozy 消毒器样本包），不得据此宣称“完整商业生产 readiness”或“任意产品线可直接上线”。
...
- **C21 live provider call** 已验证过一次样本执行路径；下一次执行仍需用户再次提供精确授权与新私有 approval/account readiness/payload 评审文件组合。
- **Provider capability evidence** 已从 no-token 扩展到一次 scoped real smoke；当前仍不代表全量模型、全场景、全流程稳定可交付。
- **Brand data assets** 已入 `pending_review`，但未形成 delivery accepted / approved brand token / publish allowed。
- **Longform production** 结构仍停留在审计对象和 blueprint 阶段；长流程全量 runtime 与发布链路仍待后续执行阶段。
- **Delivery and publish gates** intentionally remain locked without human review and acceptance evidence.
- **Toolbox authorized live**仍限制在 `sample_plan.toolbox_tool_ids` 明确列出的工具范围，未列工具继续 blocked。
