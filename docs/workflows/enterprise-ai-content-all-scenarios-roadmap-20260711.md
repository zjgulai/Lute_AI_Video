...
- [x] `completed_local / independent_review=true / not_deployed` W5-04 H3 window repair：首次 governed live window 在生产 SHA `0bb5022e2f03c212f1681789428e0e1c42b13255` 完成 exact preflight 并永久消费单次 marker；唯一 POST 在 provider account/client 之前以 HTTP 500 `submission_initialization_failed` 失败，无 poll、重发或产物，durable account absent、attempts 0，provider-off 恢复通过，publish/delivery=false。根因是窗口漏注入 mandatory server-owned `PROVIDER_JOB_BUDGET_USD`。H3 本地修复改为冻结同一 plan 字节、从其派生 canonical cap/SHA、复制同一 snapshot，并在 marker 前核对容器 plan digest/budget；同一独立线程修复 1 个 Medium plan-swap TOCTOU 后返回 `PASS / APPROVE`、`accepted_actionable_findings=0`。最终 affected `472 passed, 1 deselected`，`make ci` `4487 passed, 9 skipped, 24 deselected`。修复尚未 commit/deploy。
- [ ] `blocked_external / prior_attempt_consumed_failed` W5-04：仍需 exact H3 release promotion、全新 L4 activation/binding/raw key 和一次 Fast 视频提交，随后核对 pending-review、provider-cost、透明度及人工验收；禁止复用 2026-07-24 已消费 authority。
