#!/usr/bin/env python3
"""Run the L4D-1 single-image poyo smoke behind explicit gates."""
...
with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
    ...
    from src.pipeline.l4d_image_only_smoke import run_l4d_image_only_smoke


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--dry-run", action="store_true", help="Check gates and build the one-image job plan only.")
    mode.add_argument("--execute", action="store_true", help="Submit one poyo gpt-image-2 job after all gates pass.")
    parser.add_argument(
...
def _validate_output_path(path: Path) -> Path:
    ...
    if resolved.is_relative_to(REPO_ROOT.resolve()) and not resolved.is_relative_to(repo_tmp):
        raise ValueError("L4D image-only report output must be under tmp/ or outside the repository")
    return resolved
...
def main() -> int:
    ...
    submitter_factory = (
        (lambda: build_authorized_live_poyo_runtime_submitter(env=os.environ))
        if args.enable_poyo_http_submitter and mode == "execute"
        else None
    )
    report = run_l4d_image_only_smoke(mode=mode, env=os.environ, submitter_factory=submitter_factory)
    ...
    if args.output is None:
        ...
        try:
            ...
            print(f"Wrote L4D image-only smoke report: {output_path}")
