#!/usr/bin/env python3
"""Run the L4D-3 paired poyo image plus Seedance smoke behind explicit gates."""
...
with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
    from src.pipeline.l4d_paired_smoke import (
        build_l4d_paired_poyo_submitter_from_env,
...
def _parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="Check gates and build the paired job plan only.")
    mode.add_argument("--execute", action="store_true", help="Submit one image job and one chained video job.")
    parser.add_argument(
...
def _validate_output_path(path: Path) -> Path:
    ...
    if resolved.is_relative_to(REPO_ROOT.resolve()) and not resolved.is_relative_to(repo_tmp):
        raise ValueError("L4D paired report output must be under tmp/ or outside the repository")
    return resolved
...
def main() -> int:
    ...
    submitter_factory = (
        (lambda: build_l4d_paired_poyo_submitter_from_env(env=os.environ))
        if args.enable_poyo_http_submitter and mode == "execute"
        else None
    )
    report = run_l4d_paired_smoke(mode=mode, env=os.environ, submitter_factory=submitter_factory)
    ...
    if args.output is None:
        ...
        try:
            ...
            print(f"Wrote L4D paired smoke report: {output_path}")
