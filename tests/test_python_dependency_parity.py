...
def test_ytdlp_security_contract_is_exactly_pinned_for_ci_and_production():
    ...
    assert "yt-dlp==2026.7.4" in runtime_dependencies
    assert "yt-dlp==2026.7.4" in requirement_lines
    assert 'name = "yt-dlp"\nversion = "2026.7.4"' in UV_LOCK.read_text()


def test_requirements_is_only_the_fresh_generated_compatibility_export():
    ...
    committed_lines = REQUIREMENTS.read_text().splitlines()
    generated_lines = generated.splitlines()
    assert committed_lines[0] == "# This file was autogenerated by uv via the following command:"
    assert generated_lines[0] == committed_lines[0]
