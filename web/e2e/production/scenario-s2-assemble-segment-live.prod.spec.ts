...
const liveSpecFileName = "scenario-s2-assemble-segment-live.prod.spec.ts";
const assembleStopStep = "assemble_final";
...
const explicitStopStep = process.env.PLAYWRIGHT_S2_SEGMENTED_MEDIA_STOP_STEP ?? assembleStopStep;
const submitTimeoutMs = Number(process.env.PLAYWRIGHT_S2_ASSEMBLE_SUBMIT_TIMEOUT_MS ?? String(10 * 60_000));
...
function requiredEnv(name: string): string {
  ...
  expect(value, `${name} must be provided for refs-only assemble live smoke`).not.toBe("");
...
function buildMediaRefs(): Record<string, unknown> {
  return {
    clip_paths: [requiredEnv("PLAYWRIGHT_S2_ASSEMBLE_REF_CLIP_PATH")],
    audio_paths: [requiredEnv("PLAYWRIGHT_S2_ASSEMBLE_REF_AUDIO_PATH")],
    thumbnail_image_paths: [requiredEnv("PLAYWRIGHT_S2_ASSEMBLE_REF_THUMBNAIL_PATH")],
    lyrics_paths: optionalEnvPath("PLAYWRIGHT_S2_ASSEMBLE_REF_LYRICS_PATH"),
...
function assertPreSubmitBackendControlsReady(): void {
  ...
  expect(pipelineSource).toContain('"assemble_final": [\n        "assemble_final",\n    ]');
  expect(pipelineSource).toContain('"assemble_final": {}');
  expect(pipelineSource).toContain("_normalize_assemble_media_refs");
  expect(pipelineSource).toContain("refs_only_media_assembly");
  expect(pipelineSource).toContain("_seed_refs_only_assemble_inputs");
  expect(pipelineSource).toContain("_scope_refs_only_assemble_output");
}
...
function expectStepDone(
  ...
  expect(steps?.[stepName]?.status, `${stepName} must be done in assemble segment smoke`).toBe("done");
...
function expectStepNotExecuted(
  ...
  expect(step.status, `${stepName} must not execute in assemble segment smoke`).toBe("pending");
...
function expectSegmentedAssembleStatusReadback(statusBody: Record<string, unknown>, runLabel: string): void {
  ...
  expectReviewScopedPath(assembleOutput?.render_json_path, "assemble render_json_path");
  expect(assembleOutput?.refs_only).toBe(true);
}

function buildRunLabel(): string {
  const runId = process.env.PLAYWRIGHT_P15D_ASSEMBLE_RUN_ID
    ?? new Date().toISOString().replace(/\D/g, "").slice(0, 14);
  return `p1_s2_segmented_assemble_${runId}`;
}

test.describe("P1-5D S2 segmented assemble live provider smoke", () => {
  test("single S2 segmented assemble submit stops after assemble_final @token-smoke", async ({ request }) => {
    ...
    expect(maxSubmitCount, "P1-5D assemble segment must be capped to one scenario submit").toBe(1);
    expect(providerMaxRetries, "provider/backend retries must be disabled").toBe(0);
    expect(artifactDisposition, "assemble segment must write only pending_review artifacts").toBe("pending_review");
    expect(explicitStopStep, "P1-5D assemble live smoke must target assemble_final").toBe(assembleStopStep);
      ...
      response = await request.post("/api/scenario/s2", {
        ...
        data: {
          brand_package: {
            ...
            visual_constraints: "refs-only assembly, no new media generation",
          },
          ...
          media_stop_step: assembleStopStep,
    ...
    if (submitTimedOut) {
      ...
      expectSegmentedAssembleStatusReadback(statusBody, runLabel);
    ...
    if ([401, 403, 422, 429].includes(response!.status())) {
      throw new Error(`P1-5D stop-loss status from S2 submit: ${response!.status()} ${await response!.text()}`);
    }

    expect(response!.status(), "single S2 assemble segment submit should complete").toBe(200);
    ...
    expect(body.bounded_media_stop_step).toBe(assembleStopStep);
    expect(body.refs_only_media_assembly).toBe(true);
    ...
    expectReviewScopedPath(body.intermediate_render_json_path, "intermediate_render_json_path");
    ...
    expect(statusResponse.status(), "completed S2 assemble segment must be readable by explicit output_label").toBe(200);
    const statusBody = await statusResponse.json() as Record<string, unknown>;
    expectSegmentedAssembleStatusReadback(statusBody, runLabel);
  });
});
