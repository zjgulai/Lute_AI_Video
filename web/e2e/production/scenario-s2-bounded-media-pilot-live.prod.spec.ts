...
const liveSpecFileName = "scenario-s2-bounded-media-pilot-live.prod.spec.ts";
...
const submitTimeoutMs = Number(process.env.PLAYWRIGHT_S2_BOUNDED_SUBMIT_TIMEOUT_MS ?? String(10 * 60_000));
...
function extractSourceBlock(source: string, marker: string): string {
  ...
  const nextClass = tail.slice(marker.length).search(/\n(?:class|async def|def) /);
  if (nextClass === -1) {
    return tail;
  }
  return tail.slice(0, marker.length + nextClass);
}
...
function assertPreSubmitBackendControlsReady(): void {
  ...
  const pipeline = extractSourceBlock(pipelineSource, "class S2BrandCampaignPipeline");
  expect(pipeline).toContain("if artifact_disposition in {\"pending_review\", \"quarantine\"}");
  ...
  expect(s1Pipeline).toContain("artifact_disposition");
  expect(s1Pipeline).toContain("provider_max_retries=config.get(\"provider_max_retries\")");
  expect(s1Pipeline).toContain("image_job_cap");
  expect(s1Pipeline).toContain("video_job_cap");
  expect(s1Pipeline).toContain("quality_gate_enabled");
  expect(s1Pipeline).toContain("OUTPUT_DIR / \"tenants\" / tenant_id / disposition / label / media_kind");
  ...
  const gptImageSkill = readRepoFile("src/skills/gpt_image_generate.py");
  expect(gptImageSkill).toContain("output_dir=output_dir");
  expect(gptImageSkill).toContain("max_retries=provider_max_retries");
...
function buildRunLabel(): string {
  const runId = process.env.PLAYWRIGHT_L4D5_RUN_ID
    ?? new Date().toISOString().replace(/\D/g, "").slice(0, 14);
  return `l4d5s_s2_bounded_${runId}`;
}
...
test.describe("L4D-5 S2 bounded media live provider smoke", () => {
  test("single S2 bounded media submit stops after seedance_clips @token-smoke", async ({ request }) => {
    ...
    expect(maxSubmitCount, "L4D-5 must be capped to one scenario submit").toBe(1);
      ...
      response = await request.post("/api/scenario/s2", {
        ...
        data: {
          brand_package: {
            ...
            visual_constraints: "clean product demo, vertical short-form framing",
          },
    ...
    if ([401, 403, 422, 429].includes(response!.status())) {
      throw new Error(`L4D-5 stop-loss status from S2 submit: ${response!.status()} ${await response!.text()}`);
    }

    expect(response!.status(), "single S2 bounded media submit should complete").toBe(200);
