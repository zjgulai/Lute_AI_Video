...
const liveSpecFileName = "scenario-s3-bounded-media-pilot-live.prod.spec.ts";
...
const submitTimeoutMs = Number(process.env.PLAYWRIGHT_S3_BOUNDED_SUBMIT_TIMEOUT_MS ?? String(10 * 60_000));
...
function assertPreSubmitBackendControlsReady(): void {
  const requestModel = extractSourceBlock(readRepoFile("src/routers/_state.py"), "class S3InfluencerRemixRequest");
  ...
  const router = extractSourceBlock(readRepoFile("src/routers/scenario.py"), "async def run_s3_influencer_remix");
  expect(router).toContain("body: S3InfluencerRemixRequest");
  expect(router).toContain("artifact_disposition=body.artifact_disposition");
  expect(router).toContain("provider_max_retries=effective_provider_max_retries");
  expect(router).toContain("output_label=body.output_label");

  const pipelineSource = readRepoFile("src/pipeline/s3_remix_pipeline.py");
  expect(pipelineSource).toContain("S3_BOUNDED_MEDIA_STOP_STEP = \"seedance_clips\"");
  expect(pipelineSource).toContain("S3_BOUNDED_MEDIA_STEP_ORDER");
  ...
  const pipeline = extractSourceBlock(pipelineSource, "class S3InfluencerRemixPipeline");
  expect(pipeline).toContain("if bounded_media_pilot:");
  expect(pipeline).toContain("result.final_video_path = \"\"");
  expect(pipeline).toContain("result.publish_allowed = False");
  expect(pipeline).toContain("result.approved_brand_token_write = False");
  expect(pipeline).toContain("provider_max_retries");
  expect(pipeline).toContain("provider_job_caps");
  expect(pipeline).toContain("image_job_cap");
  expect(pipeline).toContain("video_job_cap");

  const keyframeSkill = readRepoFile("src/skills/keyframe_images.py");
  expect(keyframeSkill).toContain("generate_params[\"output_dir\"] = output_dir");
...
function buildRunLabel(): string {
  const runId = process.env.PLAYWRIGHT_P1_S3_RUN_ID
    ?? new Date().toISOString().replace(/\D/g, "").slice(0, 14);
  return `p1_s3_bounded_${runId}`;
}
...
function expectBoundedStatusReadback(statusBody: Record<string, unknown>, runLabel: string): string[] {
  ...
  expect(statusBody.scenario).toBe("s3");
...
test.describe("TODO-P1-2 S3 bounded media live provider smoke", () => {
  test("single S3 bounded media submit stops after seedance_clips @token-smoke", async ({ request }) => {
    ...
    expect(maxSubmitCount, "S3 bounded smoke must be capped to one scenario submit").toBe(1);
    ...
    try {
      response = await request.post("/api/scenario/s3", {
    ...
    if (submitTimedOut) {
      const statusResponse = await request.get(`/api/scenario/s3/status/${runLabel}`, {
        headers: productionApiHeaders(),
        timeout: 60_000,
      });
      expect(statusResponse.status(), "timed-out S3 submit must be readable by explicit output_label").toBe(200);
    ...
    expect(response, "S3 submit response must exist unless transport timeout readback passed").not.toBeNull();

    if ([401, 403, 422, 429].includes(response!.status())) {
      throw new Error(`TODO-P1-2 stop-loss status from S3 submit: ${response!.status()} ${await response!.text()}`);
    }

    expect(response!.status(), "single S3 bounded media submit should complete").toBe(200);
    const body = await response!.json();

    expect(body.success).toBe(true);
    expect(body.label).toBe(runLabel);
    expect(body.scenario).toBe("s3");
