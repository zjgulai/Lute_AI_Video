...
const liveSpecFileName = "scenario-s4-bounded-media-pilot-live.prod.spec.ts";
...
const submitTimeoutMs = Number(process.env.PLAYWRIGHT_S4_BOUNDED_SUBMIT_TIMEOUT_MS ?? String(10 * 60_000));
...
function assertPreSubmitBackendControlsReady(): void {
  const requestModel = extractSourceBlock(readRepoFile("src/routers/_state.py"), "class S4LiveShootRequest");
  ...
  const router = extractSourceBlock(readRepoFile("src/routers/scenario.py"), "async def run_s4_live_shoot");
  expect(router).toContain("body: S4LiveShootRequest");
  ...
  const pipelineSource = readRepoFile("src/pipeline/s4_live_shoot_pipeline.py");
  expect(pipelineSource).toContain("S4_BOUNDED_MEDIA_STOP_STEP = \"seedance_clips\"");
  expect(pipelineSource).toContain("S4_BOUNDED_MEDIA_STEP_ORDER");
  ...
  const pipeline = extractSourceBlock(pipelineSource, "class S4LiveShootPipeline");
...
function buildRunLabel(): string {
  const runId = process.env.PLAYWRIGHT_P1_S4_RUN_ID
    ?? new Date().toISOString().replace(/\D/g, "").slice(0, 14);
  return `p1_s4_bounded_${runId}`;
}
...
function expectBoundedStatusReadback(statusBody: Record<string, unknown>, runLabel: string): string[] {
  ...
  expect(statusBody.scenario).toBe("s4");
  ...
  expectStepNotExecuted(steps, "thumbnails");
...
test.describe("TODO-P1-3 S4 bounded media live provider smoke", () => {
  test("single S4 bounded media submit stops after seedance_clips @token-smoke", async ({ request }) => {
    ...
    expect(maxSubmitCount, "S4 bounded smoke must be capped to one scenario submit").toBe(1);
    ...
    try {
      response = await request.post("/api/scenario/s4", {
    ...
    if (submitTimedOut) {
      const statusResponse = await request.get(`/api/scenario/s4/status/${runLabel}`, {
        headers: productionApiHeaders(),
        timeout: 60_000,
      });
      expect(statusResponse.status(), "timed-out S4 submit must be readable by explicit output_label").toBe(200);
    ...
    expect(response, "S4 submit response must exist unless transport timeout readback passed").not.toBeNull();

    if ([401, 403, 422, 429].includes(response!.status())) {
      throw new Error(`TODO-P1-3 stop-loss status from S4 submit: ${response!.status()} ${await response!.text()}`);
    }

    expect(response!.status(), "single S4 bounded media submit should complete").toBe(200);
