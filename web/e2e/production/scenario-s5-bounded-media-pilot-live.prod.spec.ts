...
const liveSpecFileName = "scenario-s5-bounded-media-pilot-live.prod.spec.ts";
...
const submitTimeoutMs = Number(process.env.PLAYWRIGHT_S5_BOUNDED_SUBMIT_TIMEOUT_MS ?? String(10 * 60_000));
...
function assertPreSubmitBackendControlsReady(): void {
  const requestModel = extractSourceBlock(readRepoFile("src/routers/_state.py"), "class S5BrandVlogRequest");
  ...
  const router = extractSourceBlock(readRepoFile("src/routers/scenario.py"), "async def run_s5_brand_vlog");
  expect(router).toContain("body: S5BrandVlogRequest");
  ...
  const pipelineSource = readRepoFile("src/pipeline/s5_brand_vlog_pipeline.py");
  expect(pipelineSource).toContain("S5_BOUNDED_MEDIA_STOP_STEP = \"seedance_clips\"");
  expect(pipelineSource).toContain("S5_BOUNDED_MEDIA_STEP_ORDER");
  ...
  const pipeline = extractSourceBlock(pipelineSource, "class S5BrandVlogPipeline");
  expect(pipeline).toContain("if bounded_media_pilot:");
  expect(pipeline).toContain("\"final_video_path\": \"\"");
  expect(pipeline).toContain("\"publish_allowed\": False");
  expect(pipeline).toContain("\"approved_brand_token_write\": False");
...
function buildRunLabel(): string {
  const runId = process.env.PLAYWRIGHT_P1_S5_RUN_ID
    ?? new Date().toISOString().replace(/\D/g, "").slice(0, 14);
  return `p1_s5_bounded_${runId}`;
}
...
function expectBoundedStatusReadback(statusBody: Record<string, unknown>, runLabel: string): string[] {
  ...
  expect(statusBody.scenario).toBe("s5");
...
test.describe("TODO-P1-4 S5 bounded media live provider smoke", () => {
  test("single S5 bounded media submit stops after seedance_clips @token-smoke", async ({ request }) => {
    ...
    expect(maxSubmitCount, "S5 bounded smoke must be capped to one scenario submit").toBe(1);
    ...
    try {
      response = await request.post("/api/scenario/s5", {
        ...
        data: {
          ...
          story_description: "A calm day-in-the-life product vlog for Momcozy sterilizer.",
          video_duration: 15,
    ...
    if (submitTimedOut) {
      const statusResponse = await request.get(`/api/scenario/s5/status/${runLabel}`, {
        headers: productionApiHeaders(),
        timeout: 60_000,
      });
      expect(statusResponse.status(), "timed-out S5 submit must be readable by explicit output_label").toBe(200);
    ...
    expect(response, "S5 submit response must exist unless transport timeout readback passed").not.toBeNull();

    if ([401, 403, 422, 429].includes(response!.status())) {
      throw new Error(`TODO-P1-4 stop-loss status from S5 submit: ${response!.status()} ${await response!.text()}`);
    }

    expect(response!.status(), "single S5 bounded media submit should complete").toBe(200);
