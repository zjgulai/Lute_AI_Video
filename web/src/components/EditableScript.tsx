...
export default function EditableScript({ script, onChange }: Props) {
  ...
  return (
    <div className="apple-card p-3 bg-white space-y-3">
      {(script.segments || []).map((seg: any, i: number) => (
        <div key={i} className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold text-[#6A2B3A] uppercase">{seg.segment_type}</span>
            <span className="text-[10px] text-[#9FA0A0] font-mono">
              {seg.start_time ?? 0}s — {seg.end_time ?? 0}s
            </span>
          </div>
          <div>
            <label className="block text-[10px] font-medium text-[#9FA0A0] mb-0.5">{t("editors.voiceover")}</label>
            <textarea
              value={seg.voiceover || ""}
          ...
          <div>
            <label className="block text-[10px] font-medium text-[#9FA0A0] mb-0.5">{t("editors.visual_desc")}</label>
            <textarea
          ...
          {i < (script.segments || []).length - 1 && (
            <div className="border-t border-[#EDD3D1] pt-2" />
          )}
        </div>
      ))}
    </div>
