...
export default function PublishPanel({ videoPath, metadata, onPublished, onClose }: Props) {
  ...
  return (
    <div className="apple-card p-4 bg-white border border-[#e8e8ed]">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-[#7CB342]/10 flex items-center justify-center">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7CB342" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <h3 className="text-sm font-semibold text-[#1d1d1f]">{t("perf.publishTitle")}</h3>
        </div>
        <button
          onClick={handleClose}
      ...
      {/* Platform selection */}
      <div className="mb-3">
        <p className="text-[11px] font-medium text-[#1d1d1f] mb-1.5">{t("perf.publishSelect")}</p>
        <div className="flex gap-3">
          <label className="flex items-center gap-1.5 text-[11px] text-[#1d1d1f] cursor-pointer">
            <input
              type="checkbox"
      ...
      {/* Title field */}
      <div className="mb-2">
        <label className="text-[10px] font-medium text-[#86868b] uppercase tracking-wider block mb-1">
          {t("step.editTitle")}
        </label>
        <input
          type="text"
          ...
          className="w-full text-[12px] px-2.5 py-1.5 rounded-lg border border-[#e8e8ed] bg-white text-[#1d1d1f] placeholder-[#aeaeb2] focus:outline-none focus:border-[#7CB342] disabled:opacity-50"
        />
      </div>
      ...
      <div className="mb-3">
        ...
        <textarea
          ...
          className="w-full text-[12px] px-2.5 py-1.5 rounded-lg border border-[#e8e8ed] bg-white text-[#1d1d1f] placeholder-[#aeaeb2] focus:outline-none focus:border-[#7CB342] resize-none disabled:opacity-50"
        />
      </div>
      ...
      {Object.keys(progress).length > 0 && (
        <div className="space-y-2 mt-3">
          ...
          {Object.entries(progress).map(([platform, p]) => (
            <div key={platform} className="flex items-center justify-between p-2 rounded-lg bg-[#fafafc] border border-[#e8e8ed]">
              <div className="flex items-center gap-2">
                <PlatformStateIcon state={p.state} />
                <span className="text-[11px] font-medium text-[#1d1d1f]">
                  {platform === "tiktok" ? "TikTok" : "Shopify"}
                </span>
                <span className="text-[10px] text-[#86868b]">{stateLabel(p.state)}</span>
              </div>
      ...
      {/* Overall error */}
      {overallError && (
        <div className="mt-2 text-[10px] text-[#ff453a] bg-[#ff453a]/5 px-2 py-1.5 rounded-lg">
          {overallError}
        </div>
      )}
      ...
      {/* All done */}
      {isComplete && (
        <div className="mt-3 text-[11px] text-[#7CB342] font-medium text-center">
          {t("perf.published")}
        </div>
      )}
    </div>
...
function PlatformStateIcon({ state }: { state: ProgressState }) {
  switch (state) {
    ...
    case "processing":
      return (
        <svg className="animate-spin h-3.5 w-3.5 text-[#7CB342]" viewBox="0 0 24 24">
    ...
    case "done":
      return (
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7CB342" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
