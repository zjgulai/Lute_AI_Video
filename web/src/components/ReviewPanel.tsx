...
export default function ReviewPanel({ reviewState, currentReview, onAction, loading }: Props) {
  ...
  const renderContent = () => {
          ...
          {s.weekly_calendar.briefs.map((brief: any) => {
            ...
            return (
              <div key={brief.id} className="apple-card overflow-hidden">
                ...
                >
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-[#7CB342]/10 text-[#7CB342] shrink-0">
                  ...
                  >
                    <path d="M2.5 3.5L5 6.5L7.5 3.5" stroke="#aeaeb2" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </button>
                {exp && (
                  <div className="px-3 pb-3 space-y-2 animate-slide-down">
                    <p className="text-xs text-[#86868b]">{brief.key_message}</p>
                    {brief.usp_priority?.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {brief.usp_priority.map((usp: string, i: number) => (
                          <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-[#f5f5f7] text-[#86868b] border border-[#e8e8ed]">
                            {usp}
                          </span>
                        ))}
                      </div>
                    ...
                    {brief.target_audience && (
                      <p className="text-[11px] text-[#aeaeb2]">{t("review.targetAudience")}: {brief.target_audience}</p>
                    )}
                  </div>
                )}
              </div>
    ...
    if (currentReview === "script_review" && s?.scripts) {
      return (
        <div className="space-y-2">
          <p className="text-[11px] text-[#86868b] mb-3">{t("review.scripts")} {s.scripts.length}{t("review.scriptsCount")}</p>
            ...
            return (
              <div key={key} className="apple-card overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center gap-2 p-3 cursor-pointer text-left"
                >
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-[#86868b]/10 text-[#86868b] shrink-0">
                    {t("platform." + script.platform) || script.platform}
                  </span>
                  <span className="text-[11px] text-[#aeaeb2] font-mono">{script.id}</span>
                  <span className="text-[11px] text-[#aeaeb2]">{script.total_duration}s</span>
                  <svg
                    width="10" height="10" viewBox="0 0 10 10" fill="none"
                    className={`ml-auto shrink-0 transition-transform ${exp ? "rotate-180" : ""}`}
                  >
                ...
                {exp && script.segments?.map((seg: any, i: number) => (
                  <div key={i} className="px-3 pb-2 last:pb-3">
                    <div className="pl-3 border-l-2 border-[#e8e8ed]">
                      ...
                      <p className="text-[13px] text-[#1d1d1f] leading-relaxed">{seg.voiceover}</p>
                      {seg.visual_description && (
                        <div className="flex items-start gap-1.5 mt-1 text-[11px] text-[#aeaeb2]">
                          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="mt-0.5 shrink-0">
                            <rect x="1" y="2" width="10" height="8" rx="1" stroke="#aeaeb2" strokeWidth="0.8" />
                            <circle cx="4.5" cy="5.5" r="1.5" stroke="#aeaeb2" strokeWidth="0.8" />
                            <path d="M1 8l2.5-2 2 1.5L8 5l3 3" stroke="#aeaeb2" strokeWidth="0.8" />
                          </svg>
                          {seg.visual_description}
                        </div>
                      )}
        ...
        <div className="space-y-3">
          {s.thumbnail_sets.map((ts: any, idx: number) => (
            <div key={`ts-${idx}`} className="apple-card p-3 space-y-2">
              <p className="text-[11px] font-semibold text-[#86868b]">{ts.script_id}</p>
                  ...
                  return (
                    <button
                      ...
                      className={`apple-card p-3 text-left cursor-pointer transition-all ${
                        selected ? "ring-2 ring-[#7CB342] ring-offset-2" : "hover:shadow-md"
                      }`}
                    >
                      <div className="bg-gradient-to-br from-[#f5f5f7] to-[#e8e8ed] rounded-lg h-20 flex items-center justify-center mb-2">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="#aeaeb2">
                          <rect x="3" y="3" width="18" height="18" rx="3" />
                          <circle cx="9" cy="9" r="3" />
                          <path d="M3 15l4.5-4 3.5 3 4.5-4.5L21 15" />
                        </svg>
                      </div>
                      <div className="text-xs font-semibold text-[#1d1d1f]">{t("review.variant")} {v.variant_id}</div>
                      <div className="text-[10px] text-[#86868b] line-clamp-2">{v.concept}</div>
                      {selected && (
                        <div className="text-[10px] font-semibold text-[#7CB342] mt-1 flex items-center gap-1">
                          <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                            <path d="M2 5.5L4 7.5L8 2.5" stroke="#7CB342" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                          {t("review.selected")}
                        </div>
                      )}
    ...
    // ── Fallback: raw data ──
    return (
      <details className="text-xs text-[#aeaeb2] cursor-pointer">
        <summary>{t("review.viewRaw")}</summary>
        <pre className="mt-2 p-3 rounded-lg bg-[#f5f5f7] overflow-auto text-[10px] max-h-60 border border-[#e8e8ed]">
      ...
      <div className="apple-card overflow-hidden">
        <div className="p-3 space-y-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-[#1d1d1f]">{t(config.titleKey)}</h2>
              <p className="text-xs text-[#86868b] mt-0.5">{t(config.subtitleKey)}</p>
            </div>
          </div>
          <p className="text-xs text-[#aeaeb2] leading-relaxed">{t(config.descKey)}</p>

          {/* Audit Score */}
          {auditReport ? (
            <AuditScoreCard report={auditReport} />
          ) : (
            <div className="text-center py-4 rounded-xl bg-[#f5f5f7] border border-[#e8e8ed]">
              <p className="text-xs text-[#aeaeb2]">{t("review.noAudit")}</p>
            </div>
          )}
          ...
          {/* Decision */}
          <div className="border-t border-[#e8e8ed] pt-3 space-y-2">
            <textarea
              value={notes}
