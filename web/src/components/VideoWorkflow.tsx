...
export default function VideoWorkflow({
  ...
  return (
    <div className="space-y-3 animate-slide-up">
      ...
      <div className="apple-card p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#7CB342]/10 flex items-center justify-center">
            ...
            <div>
              <h2 className="text-sm font-semibold text-[#1d1d1f]">{t("workflow.title")}</h2>
              <p className="text-[10px] text-[#86868b]">Label: {label}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-[#f5f5f7] rounded-lg p-2">
            <p className="text-[9px] text-[#86868b] uppercase">{t("workflow.product")}</p>
            <p className="text-xs font-medium text-[#1d1d1f] truncate">{productName}</p>
          </div>
          <div className="bg-[#f5f5f7] rounded-lg p-2">
            <p className="text-[9px] text-[#86868b] uppercase">{t("workflow.brand")}</p>
            <p className="text-xs font-medium text-[#1d1d1f]">{brandName || "-"}</p>
          </div>
          <div className="bg-[#f5f5f7] rounded-lg p-2">
            <p className="text-[9px] text-[#86868b] uppercase">{t("workflow.duration")}</p>
            <p className="text-xs font-medium text-[#1d1d1f]">{duration}s</p>
          </div>
          <div className="bg-[#f5f5f7] rounded-lg p-2">
            <p className="text-[9px] text-[#86868b] uppercase">{t("workflow.scenario")}</p>
            <p className="text-xs font-medium text-[#1d1d1f]">{scenarioLabel}</p>
          </div>
        </div>
        ...
        {platforms.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {platforms.map((p: string) => (
              <span key={p} className="text-[10px] px-2 py-0.5 rounded-full bg-[#7CB342]/10 text-[#7CB342] border border-[#7CB342]/15">
                {t("platform." + p)}
              </span>
            ))}
          </div>
      ...
      <div className="apple-card p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-semibold text-[#1d1d1f]">{t("workflow.timeline")}</h3>
              ...
              <div key={stepName} className="space-y-1">
                <div
                  className={`flex items-center gap-2 p-2.5 rounded-lg border transition-all ${
                    hasError
                      ? "bg-[#fff5f5] border-[#ff453a] ring-1 ring-[#ff453a]/10"
                      : isDone
                      ? isEdited
                        ? "bg-[#7CB342]/5 border-[#7CB342]/30 ring-1 ring-[#7CB342]/10"
                        : "bg-[#f5f5f7] border-[#e8e8ed]"
                      : isCurrent
                      ? "bg-white border-[#7CB342] ring-1 ring-[#7CB342]/20"
                      : "bg-[#fafafc] border-[#e8e8ed] opacity-50"
                  }`}
                >
                  <span className={`text-[10px] font-mono w-5 text-center ${isDone ? "text-[#7CB342]" : isCurrent ? "text-[#ff9500]" : "text-[#aeaeb2]"}`}>
                    {isDone ? (
                      <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                        <path d="M4 8.5L7 11.5L12 5" stroke="#7CB342" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    ) : (
                      index + 1
                    )}
                  ...
                  <div className={`w-2 h-2 rounded-full shrink-0 ${
                    hasError ? "bg-[#ff453a]" : isDone ? "bg-[#7CB342]" : isCurrent ? "bg-[#ff9500] animate-pulse" : "bg-[#e8e8ed]"
                  }`} />

                  <span className={`text-xs font-medium flex-1 ${hasError ? "text-[#ff453a]" : isDone ? "text-[#1d1d1f]" : isCurrent ? "text-[#1d1d1f]" : "text-[#aeaeb2]"}`}>
                    {t("wstep." + stepName)}
                  </span>

                  {isEdited && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#7CB342]/10 text-[#7CB342] font-medium">{t("stepStatus.edited")}</span>
                  )}

                  <span className="text-[9px] text-[#aeaeb2] hidden sm:inline">{getStepDurationLabel(stepName)}</span>

                  <span className={`text-[10px] font-medium ${hasError ? "text-[#ff453a]" : isDone ? "text-[#7CB342]" : isCurrent ? "text-[#ff9500]" : "text-[#aeaeb2]"}`}>
                  ...
                  {isDone && !isRunning && (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="text-[10px] text-[#ff9500] hover:underline cursor-pointer px-1.5 py-0.5 rounded hover:bg-[#ff9500]/5"
                      >
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="text-[10px] text-[#86868b] hover:text-[#1d1d1f] cursor-pointer px-1.5 py-0.5 rounded hover:bg-[#e8e8ed]/50 disabled:opacity-50"
                      >
                  ...
                  {isCurrent && (
                    <button
                      ...
                      disabled={loading}
                      className="apple-btn apple-btn-primary text-[10px] px-2.5 py-1 disabled:opacity-50"
                    >
                      {isRunning ? (
                  ...
                  {hasError && (
                    <button
                      onClick={() => handleRunStep(stepName)}
                      disabled={loading}
                      className="apple-btn text-[10px] px-2.5 py-1 bg-[#ff453a] hover:bg-[#e03a30] text-white disabled:opacity-50"
                    >
                      {t("waction.retry")}
                    </button>
                  )}
                ...
                {hasError && state?.errors?.filter((e: string) => e.includes(stepName)).map((err: string, i: number) => (
                  <div key={i} className="ml-7 p-2 bg-[#fff5f5] rounded-lg border border-[#ff453a]/20">
                    <p className="text-[10px] text-[#ff453a]">{err}</p>
                  </div>
                ))}
              </div>
            );
        ...
        <div className="mt-4 pt-3 border-t border-[#e8e8ed] flex gap-2">
          {!allDone && (
            <button
          ...
          <button
            onClick={onReset}
            disabled={loading}
            className="apple-btn py-2 text-sm px-4 bg-[#f5f5f7] text-[#86868b] hover:text-[#1d1d1f] hover:bg-[#e8e8ed] disabled:opacity-50"
          >
            {t("waction.abandon")}
          </button>
        </div>
...
function StepEditor({ stepName, output, onSave, onCancel }: {
  ...
  if (stepName === "strategy" || stepName === "compliance") {
    ...
    return (
      <div className="space-y-2 p-2">
        <p className="text-[10px] text-[#86868b] mb-1">{te("editors.briefs")}</p>
        {briefs.map((b: any, i: number) => (
          <div key={i} className="apple-card p-3 bg-white space-y-2">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-[#aeaeb2]">#{i + 1}</span>
              <span className="text-[10px] font-semibold text-[#7CB342]">{b.id || `BRIEF-${String(i + 1).padStart(3, "0")}`}</span>
            </div>
            <div>
              <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.topic")}</label>
              <input
                type="text"
                value={b.topic || ""}
                onChange={(e) => {
            ...
            <div>
              <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.target_audience")}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.key_message")}</label>
              <textarea
                value={b.key_message || ""}
                onChange={(e) => {
                  const newBriefs = [...briefs];
            ...
            <div>
              <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.hook_type")}</label>
              <input
                type="text"
                value={b.hook_type || ""}
                onChange={(e) => {
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={onCancel} className="apple-btn text-xs px-3 py-1.5 bg-[#f5f5f7] text-[#86868b]">{te("editors.cancel")}</button>
        </div>
      </div>
    );
  }
  ...
  if (stepName === "scripts") {
    ...
    return (
      <div className="space-y-2 p-2">
        <p className="text-[10px] text-[#86868b] mb-1">{te("editors.scripts")}</p>
        {scripts.map((s: any, si: number) => (
          <div key={si} className="apple-card p-3 bg-white space-y-2">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-mono text-[#aeaeb2]">{s.id || `S${si + 1}`}</span>
              <span className="text-xs font-semibold text-[#1d1d1f]">{s.product_name || s.brand_name || "Script"}</span>
            </div>
            {(s.segments || []).map((seg: any, j: number) => (
              <div key={j} className="pl-3 border-l-2 border-[#e8e8ed] space-y-1.5 py-1">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-semibold text-[#7CB342] uppercase">{seg.segment_type}</span>
                  <span className="text-[10px] text-[#aeaeb2] font-mono">{seg.start_time ?? 0}s — {seg.end_time ?? 0}s</span>
                </div>
                <div>
                  <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.voiceover")}</label>
                  <textarea
                    value={seg.voiceover || ""}
                    onChange={(e) => {
                ...
                <div>
                  <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.visual_desc")}</label>
                  <textarea
                ...
                <div className="mt-2">
                  <label className="text-[10px] font-medium text-[#86868b] mb-1 block">{te("editors.text_overlay")}</label>
                  <input
                    type="text"
                    ...
                    className="w-full bg-[#fafafc] border border-[#e8e8ed] rounded-lg px-3 py-1.5 text-[13px] text-[#1d1d1f] focus:outline-none focus:border-[#7CB342] transition-colors"
                  />
                </div>
              </div>
            ))}
  ...
  if (stepName === "storyboards") {
    ...
    return (
      <div className="space-y-2 p-2">
        <p className="text-[10px] text-[#86868b] mb-1">{te("editors.storyboards")}</p>
        {boards.map((b: any, i: number) => (
          <div key={i} className="apple-card p-3 bg-white space-y-2">
            ...
            <div>
              <label className="block text-[10px] font-medium text-[#86868b] mb-0.5">{te("editors.shot_type")}</label>
              <input
                type="text"
                value={b.shot_type || ""}
                onChange={(e) => {
  ...
  if (stepName === "video_prompts" || stepName === "thumbnail_prompts") {
    ...
    return (
      <div className="space-y-2 p-2">
        <p className="text-[10px] text-[#86868b] mb-1">{te("editors.prompts")}</p>
        {prompts.map((p: any, i: number) => (
          <div key={i} className="apple-card p-3 bg-white space-y-2">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-mono text-[#aeaeb2]">#{i + 1}</span>
              {p.platform && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#7CB342]/10 text-[#7CB342]">
  ...
  return (
    <div className="space-y-2 p-2">
      <p className="text-[10px] text-[#86868b] mb-1">{te("editors.json")}</p>
      <textarea
...
function StepOutput({ stepName, output }: { stepName: string; output: any }) {
  const { t: to } = useI18n();
  if (!output) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noOutput")}</p>;

  if (stepName === "strategy" || stepName === "compliance") {
    const briefs = Array.isArray(output) ? output : output.briefs || [];
    if (briefs.length === 0) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noStrategy")}</p>;
    return (
      <div className="space-y-2 p-2">
        {briefs.map((b: any, i: number) => (
          <div key={i} className="apple-card p-3 bg-[#fafafc]">
            ...
            <h4 className="text-sm font-semibold text-[#1d1d1f] mb-1">{b.topic || b.product_name || b.brand_name || "Brief"}</h4>
            {b.target_audience && <p className="text-[10px] text-[#aeaeb2] mb-1">{to("editors.target_audience")}:{b.target_audience}</p>}
            {b.key_message && <p className="text-xs text-[#86868b] leading-relaxed">{b.key_message}</p>}
            {b.description && <p className="text-xs text-[#86868b] leading-relaxed mt-1">{b.description}</p>}
          </div>
        ))}
      </div>
    );
  ...
  if (stepName === "scripts") {
    ...
    if (scripts.length === 0) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noScript")}</p>;
    return (
      <div className="space-y-2 p-2">
        {scripts.map((s: any, i: number) => (
          <details key={i} className="apple-card overflow-hidden">
            <summary className="p-3 cursor-pointer flex items-center gap-2 list-none">
              ...
              <span className="text-[10px] text-[#aeaeb2]">{(s.segments || []).length}{to("step.segments")}</span>
            </summary>
            <div className="px-3 pb-3 space-y-2 border-t border-[#e8e8ed] pt-2">
              {(s.segments || []).map((seg: any, j: number) => (
                <div key={j} className="pl-3 border-l-2 border-[#e8e8ed]">
                  ...
                  <p className="text-xs text-[#1d1d1f]">{seg.voiceover}</p>
                  {seg.visual_description && <p className="text-[11px] text-[#86868b] mt-1 italic">{seg.visual_description}</p>}
                  {seg.text_overlay && (
                    <p className="text-[10px] text-[#7CB342] font-medium mt-1 bg-[#f0f7e8] px-2 py-0.5 rounded-md inline-block">
                      {seg.text_overlay}
                    </p>
                  )}
                </div>
  ...
  if (stepName === "storyboards") {
    ...
    if (boards.length === 0) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noStoryboard")}</p>;
    return (
      <div className="space-y-2 p-2">
        {boards.map((b: any, i: number) => (
          <div key={i} className="apple-card p-3 bg-[#fafafc]">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-mono text-[#aeaeb2]">#{i + 1}</span>
              <span className="text-xs font-semibold text-[#1d1d1f]">{b.scene_title || "Scene"}</span>
            </div>
            {b.visual_description && <p className="text-[11px] text-[#86868b] italic">{b.visual_description}</p>}
            {b.shot_type && <p className="text-[10px] text-[#aeaeb2] mt-1">{to("editors.shot_type")}:{b.shot_type}</p>}
          </div>
        ))}
      </div>
    );
  ...
  if (stepName === "seedance_clips") {
    ...
    if (urls.length === 0) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noMedia")}</p>;
    return (
      <div className="space-y-2 p-2">
        {isNewFormat && targetDur > 0 && (
          <div className="flex items-center gap-2 text-[10px]">
            ...
            <span className={`px-1.5 py-0.5 rounded-full font-medium ${totalDur >= targetDur * 0.8 ? "bg-[#7CB342]/10 text-[#7CB342]" : "bg-[#ff9500]/10 text-[#ff9500]"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {urls.map((url: string, i: number) => {
            ...
            return (
              <div key={i} className={`apple-card overflow-hidden ${isStub ? "border-[#ff9500]/30" : ""} ${isFiller ? "border-[#5B8DEF]/30" : ""}`}>
                <video src={url} controls className="w-full h-32 object-cover" />
                <div className="p-2 space-y-1">
                  <div className="flex items-center gap-1 flex-wrap">
                    <span className="text-[9px] font-mono text-[#aeaeb2]">#{i + 1}</span>
                    {meta.duration > 0 && (
                      <span className="text-[9px] text-[#86868b]">{meta.duration.toFixed(1)}s</span>
                    )}
                    {isStub && (
                    ...
                    {isFiller && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#5B8DEF]/10 text-[#5B8DEF] font-medium">
                        {to("step.filler")}
                      </span>
                    )}
                    {hasContinuity && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#7CB342]/10 text-[#7CB342] font-medium">
                        {to("step.continuity")}
                      </span>
                    )}
                    {!verOk && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#ff453a]/10 text-[#ff453a] font-medium">
                        {to("step.verifyFail")}
                      </span>
                    )}
                  </div>
                  <p className="text-[9px] text-[#aeaeb2] truncate">{url}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-2 gap-2 p-2">
        {urls.map((url: string, i: number) => (
          <div key={i} className="apple-card overflow-hidden">
            ...
            <p className="text-[9px] text-[#aeaeb2] p-2 truncate">{url}</p>
          </div>
        ))}
      </div>
    );
  ...
  if (stepName === "assemble_final") {
    ...
    if (!finalUrl) return <p className="text-xs text-[#aeaeb2] p-2">{to("step.noData")}</p>;
    return (
      <div className="p-2">
        <div className="apple-card overflow-hidden">
          <video src={finalUrl} controls className="w-full" />
          <div className="p-2">
            <a href={finalUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#7CB342] hover:underline">
